...
MimosaSimuTrajectoryHit::MimosaSimuTrajectoryHit()
    : G4VHit(), fTrackID(-1), fTrackPDGCode(-1), fTrackVertexPos(G4ThreeVector()),
      fTrackVertexTime(-1), fEdep(0.), fFirstStepPos(G4ThreeVector()), fFirstStepTime(0.0),
      fLastStepPos(G4ThreeVector()), fLastStepTime(0.0),
...
MimosaSimuTrajectoryHit::MimosaSimuTrajectoryHit(const MimosaSimuTrajectoryHit &right) : G4VHit() {
...
const MimosaSimuTrajectoryHit &MimosaSimuTrajectoryHit::
    operator=(const MimosaSimuTrajectoryHit &right) {
    fTrackID         = right.fTrackID;
    fTrackPDGCode    = right.fTrackPDGCode;
    fTrackVertexPos  = right.fTrackVertexPos;
    fTrackVertexTime = right.fTrackVertexTime;
...
void MimosaSimuTrajectoryHit::Print() {
    ...
    G4cout << "  trackID: " << fTrackID << G4endl << "  trackPDG: " << fTrackPDGCode << G4endl
           << "  trackVertexPos[(x,y,z);y]: (" << fTrackVertexPos.x() / cm << ","
           << fTrackVertexPos.y() / cm << "," << fTrackVertexPos.z() / cm << ") cm; "
           << fTrackVertexTime / ns << " ns]" << G4endl
           << "  Edep:                            " << std::setw(Ndigits) << fEdep / keV << " keV"
           << G4endl << "  1st- step-Position[(x,y,z);t]:   [(" << std::setw(Ndigits)
           << fFirstStepPos.x() / cm << "," << fFirstStepPos.y() / cm << ","
           << fFirstStepPos.z() / cm << ") cm; " << fFirstStepTime / ns << " ns]" << G4endl
           << "  last-step-Position[(x,y,z);t]:   [(" << std::setw(Ndigits) << fLastStepPos.x() / cm
           << "," << fLastStepPos.y() / cm << "," << fLastStepPos.z() / cm << ") cm; "
           << fLastStepTime / ns << " ns]" << G4endl << "  1st -step-4Momentum(px,py,pz,E): ("
           << std::setw(Ndigits) << fFirstStep4Vector.px() / GeV << ","
           << fFirstStep4Vector.py() / GeV << "," << fFirstStep4Vector.pz() / GeV << ","
           << fFirstStep4Vector.e() / GeV << ") GeV" << G4endl
           << "  last-step-4Momentum(px,py,pz,E): (" << std::setw(Ndigits)
