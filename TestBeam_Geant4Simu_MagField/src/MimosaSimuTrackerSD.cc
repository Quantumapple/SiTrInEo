...
MimosaSimuTrackerSD::MimosaSimuTrackerSD(const G4String &name, const G4String &hitsCollectionName,
                                         const G4int TheSensorID, const bool verbose)
    : G4VSensitiveDetector(name), fHitsCollection(NULL), sensorID(TheSensorID), verbosity(verbose)
...
G4bool MimosaSimuTrackerSD::ProcessHits(G4Step *aStep, G4TouchableHistory *) {
    ...
    if (preStepPoint->GetStepStatus() == fGeomBoundary) {
        ...
        G4ThreeVector localDirection =
            theTouchable->GetHistory()->GetTopTransform().TransformAxis(worldDirection);
        ...
        if (TMath::Abs(localDirection.x()) < epsilon) {
            if (TMath::Abs(localDirection.y()) < epsilon)
                phi_local = 0.0;
            else if (localDirection.y() > 0.0)
                phi_local = TMath::Pi() / 2.0;
            else if (localDirection.y() < 0.0)
                phi_local = -TMath::Pi() / 2.0;
        } else {
            ...
            if (localDirection.x() > 0) {
                ...
                if (TMath::Abs(localDirection.y()) < epsilon)
                    ...
                    phi_local = TMath::Pi() - phi_temp;
                else if (localDirection.y() < 0.0)
                    phi_local = -TMath::Pi() + phi_temp;
            }
        }
        fFirstStepLocalAngles = G4ThreeVector(theta_local, phi_local, 0.0);
        // cout << "theta_local = " << theta_local/deg << ", Z-direction     = "  <<
        // localDirection.z() << endl; cout << "phi_local   = " << phi_local/deg   << ",
        // (X,Y)-direction = (" << localDirection.x() << "," << localDirection.y() << ")" << endl;

        G4ThreeVector localINPosition =
            theTouchable->GetHistory()->GetTopTransform().TransformPoint(preStepPointPosition);
    ...
    if (postStepPoint->GetStepStatus() == fGeomBoundary || aTrack->GetTrackStatus() != fAlive) {
        if (aTrack->GetTrackStatus() != fAlive && verbosity)
            G4cout << "The track status is different from fAlive!!!" << G4endl;
        ...
        G4ThreeVector localOUTPosition =
        ...
        if (verbosity) {
            ...
            G4cout << "PreStepPos  = (" << fFirstStepPos[0] / um << "," << fFirstStepPos[1] / um
                   << "," << fFirstStepPos[2] / um << ") um" << G4endl;
            G4cout << "PostStepPos = (" << fLastStepPos[0] / um << "," << fLastStepPos[1] / um
                   << "," << fLastStepPos[2] / um << ") um" << G4endl;
        }
