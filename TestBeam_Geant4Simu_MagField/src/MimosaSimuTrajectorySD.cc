...
MimosaSimuTrajectorySD::MimosaSimuTrajectorySD(const G4String &name,
                                               ...
                                               const bool verbose)
    : G4VSensitiveDetector(name), fTrajectoryHitsCollection(NULL), verbosity(verbose)
...
void MimosaSimuTrajectorySD::Initialize(G4HCofThisEvent *hce) {
    ...
    fTrajectoryHitsCollection =
        new MimosaSimuTrajectoryHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool MimosaSimuTrajectorySD::ProcessHits(G4Step *aStep, G4TouchableHistory *) {
    ...
    G4LorentzVector preStep4Vector =
        G4LorentzVector(preStepPoint->GetMomentum().x(), preStepPoint->GetMomentum().y(),
                        preStepPoint->GetMomentum().z(), preStepPoint->GetTotalEnergy());
    G4LorentzVector postStep4Vector =
        G4LorentzVector(postStepPoint->GetMomentum().x(), postStepPoint->GetMomentum().y(),
                        postStepPoint->GetMomentum().z(), postStepPoint->GetTotalEnergy());
...
void MimosaSimuTrajectorySD::EndOfEvent(G4HCofThisEvent *) {
    ...
    if (verbosity) {
        for (G4int i = 0; i < nofHits; i++)
            (*fTrajectoryHitsCollection)[i]->Print();
    }
}
