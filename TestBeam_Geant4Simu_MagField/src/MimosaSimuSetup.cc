...
void MimosaSimuSetup::ReadRunParameters() {
    ...
    do {
        ...
        // VR 2014/06/30 RunNumber is set from DSession
            ...
            cout << "WARNING : parameter 'RunNumber' in config file is ignored, because it is "
                    "redundant with InitSession command argument !"
                 << endl;
            getRidOfLine();
            ...
            if (strcmp(fFieldName, "Planes") && strcmp(fFieldName, "Ladders")) {
                cout << "WARNING : parameter '" << fFieldName
                     << "' in config file is not understood !" << endl;
                getRidOfLine();
            }
        }
...
void MimosaSimuSetup::ReadTrackerParameters() {
    ...
    // Modified: VR 2014/10/30 add the PreTrackHitsNbMinimum and HitsNbMinForFullTrack parameters
    // and test of mandatory config Modified: VR 2014/12/18 modify all parameters for track_finder 2
    // (IVI) Modified: JB 2014/12/23 add Subtrack parameters Modified: LC 2015/01/?? add
    // HitMonteCarlo Modified: LC 2015/01/31 add DprecAlignMethod
    ...
    TrackerParameter.PlanesForTrackMinimum =
        -1; //[MANDATORY if TracksMaximum!=0 && TracksFinder==0 ]
    TrackerParameter.HitsInPlaneTrackMaximum =
        -1; //[MANDATORY if TracksMaximum!=0 && TracksFinder==0 ]
    ...
    do {
        ...
        } else if (!strcmp(fFieldName, "ExpSetup")) { // AP 2016/07/08, Experimental setup: e.g.
                                                      // Beam-Test, sources, other ...
        ...
        // Beam-Test experimental setup parameters
            ...
            if (MimosaSimuSetupDebug)
                cout << "  BeamDirection X = " << TrackerParameter.BeamDirection(0)
                     << ",  Y = " << TrackerParameter.BeamDirection(1)
                     << ", Z = " << TrackerParameter.BeamDirection(1) << endl;
            ...
            if (MimosaSimuSetupDebug)
                cout << "  BeamOrigin X = " << TrackerParameter.BeamOrigin(0)
                     << ",  Y = " << TrackerParameter.BeamOrigin(1)
                     << ", Z = " << TrackerParameter.BeamOrigin(1) << endl;
        } else if (!strcmp(fFieldName, "BeamOriginSpreadX")) {
            Float_t temporal;
            read_item(temporal);
            TrackerParameter.BeamOriginSpread.setX(temporal *
                                                   1000); // move from millimeters to microns
        } else if (!strcmp(fFieldName, "BeamOriginSpreadY")) {
            Float_t temporal;
            read_item(temporal);
            TrackerParameter.BeamOriginSpread.setY(temporal *
                                                   1000); // move from millimeters to microns
        }
        // Source experimental setup parameters
            ...
            if (MimosaSimuSetupDebug)
                cout << "  SourcePosition X = " << TrackerParameter.SourcePosition(0)
                     << ",  Y = " << TrackerParameter.SourcePosition(1)
                     << ", Z = " << TrackerParameter.SourcePosition(1) << endl;
            ...
            if (MimosaSimuSetupDebug)
                cout << "  SourceTilt X = " << TrackerParameter.SourcePosition(2)
                     << ",  Y = " << TrackerParameter.SourceTilt(1)
                     << ", Z = " << TrackerParameter.SourceTilt(0) << endl;
        ...
        // Magnetic field, assumed constant all over the world volume
            ...
            if (MimosaSimuSetupDebug)
                cout << "  BField direction X = " << TrackerParameter.BFieldDirection(0)
                     << ",  Y = " << TrackerParameter.BFieldDirection(1)
                     << ", Z = " << TrackerParameter.BFieldDirection(1) << endl;
        }

        else if (!strcmp(fFieldName,
                         "MediumMaterial")) { // 2015/06/01, AP Material of the medium containing
                                              // the sensors: e.g. Dry air, or Vacuum
            ...
            if (TrackerParameter.TrackingPass > 0) {
                ...
                while (kTRUE) {
                    ...
                    if (nbOfReadingLoop > 20) {
                        ...
                        if (!TrackerParameter.PreTrackHitsNbMinimum)
                            cout << "          * PreTrackHitsNbMinimum  list is not defined !"
                                 << endl;
                        if (!TrackerParameter.PreTrackHitsTypeUsed)
                            cout << "          * PreTrackHitsTypeUsed   list is not defined !"
                                 << endl;
                        if (!TrackerParameter.PreTrackHitsMaxDist)
                            cout << "          * PreTrackHitsMaxDist    list is not defined !"
                                 << endl;
                        if (!TrackerParameter.ExtTrackHitsNbMinimum)
                            cout << "          * ExtTrackHitsNbMinimum  list is not defined !"
                                 << endl;
                        if (!TrackerParameter.ExtTrackHitsTypeUsed)
                            cout << "          * ExtTrackHitsTypeUsed   list is not defined !"
                                 << endl;
                        if (!TrackerParameter.ExtTrackHitsMaxDist)
                            cout << "          * ExtTrackHitsMaxDist    list is not defined !"
                                 << endl;
                        if (!TrackerParameter.FullTrackHitsNbMinimum)
                            cout << "          * FullTrackHitsNbMinimum list is not defined !"
                                 << endl;
                    ...
                    if (!strcmp(fFieldName, "PreTrackHitsNbMinimum")) {
                        TrackerParameter.PreTrackHitsNbMinimum =
                            new Int_t[TrackerParameter.TrackingPass];
                        ...
                        TrackerParameter.PreTrackHitsMaxDist =
                            new Float_t[TrackerParameter.TrackingPass];
        ...
        // -*-*-*- Tracking Order -*-*-*-
            ...
            if (TrackerParameter.TrackingOrderLines > 0) {
                TrackerParameter.TrackingOrderPreTrack =
                    new Int_t *[TrackerParameter.TrackingOrderLines];
                TrackerParameter.TrackingOrderExtTrack =
                    new Int_t *[TrackerParameter.TrackingOrderLines];
                ...
                for (Int_t iOrder = 1; iOrder <= TrackerParameter.TrackingOrderLines; iOrder++) {
                    ...
                    if (!strcmp(fFieldName, field)) {
                        ...
                        TrackerParameter.TrackingOrderPreTrack[iOrder - 1] =
                            new Int_t[aNbOfPlanes + 1];
                        ...
                        TrackerParameter.TrackingOrderExtTrack[iOrder - 1] =
                            new Int_t[aNbOfPlanes + 1];
                        ...
                        cout << "        -> Error : can't find line " << field << " in config file"
                             << endl;
                        gApplication->Terminate();
                    }
                }
        ...
        // -*-*-*- Subtrack -*-*-*-
        // JB 2014/12/23
        else if (!strcmp(fFieldName, "SubtrackNplanes") ||
                 !strcmp(fFieldName, "SubtrackNPlanes")) { // if subtrack
            ...
            if (TrackerParameter.SubtrackNplanes > 0) {
                ...
                cout << "  Creating subtrack with " << TrackerParameter.SubtrackNplanes
                     << " planes requested" << endl
                     << "   -> list of planes for subtrack: ";
                nextField();
                if (!strcmp(fFieldName, "SubtrackPlanes")) {
                    ...
                    cout << " WARNING: did not detect 'SubtrackPlanes' field so switching "
                            "SubtrackNplanes to 0!"
                         << endl;
        ...
        nextField();
             ...
             strcmp(fFieldName, "GeometryName"));

    // Check the correct setup of the experimental parameters
    CheckExpSetupParameters();

    if (TrackerParameter.FillNonSensitiveBranch)
        cout << " Filling up the Non-sensitive branch of output tree!!!" << endl;

    cout << endl;
    cout << "Bfield parameters" << endl;
    if (TrackerParameter.BFieldMagnitude == 0.0)
        cout << "    BFieldMagnitude    parameter not specified. Setting it to defaul value: "
             << TrackerParameter.BFieldMagnitude << " Tesla" << endl;
    else {
        if (TrackerParameter.BFieldMagnitude < 0) {
            TrackerParameter.BFieldMagnitude = TMath::Abs(TrackerParameter.BFieldMagnitude);
            cout << "    BFieldMagnitude    parameter set to user value is negative. Using its "
                    "absolute value: "
                 << TrackerParameter.BFieldMagnitude << " Tesla" << endl;
        } else
            cout << "    BFieldMagnitude    parameter set to user value: "
                 << TrackerParameter.BFieldMagnitude << " Tesla" << endl;
    }
    if (TrackerParameter.BFieldDirection == G4ThreeVector(0.0, 0.0, 1.0)) {
        cout << "    BFieldDirection    parameter not specified. Setting it to defaul value: ("
             << TrackerParameter.BFieldDirection(0) << "," << TrackerParameter.BFieldDirection(1)
             << "," << TrackerParameter.BFieldDirection(2) << ")" << endl;
    } else {
        // Normalizing Bfield direction vector to unit module
        TrackerParameter.BFieldDirection /= TrackerParameter.BFieldDirection.mag();
        cout << "    BFieldDirection    parameter set to user value: ("
             << TrackerParameter.BFieldDirection(0) << "," << TrackerParameter.BFieldDirection(1)
             << "," << TrackerParameter.BFieldDirection(2) << ")" << endl;
    }
    ...
    if (TrackerParameter.MediumMaterial == TString("")) {
        ...
        cout << "MediumMaterial     parameter not specified. Setting it to defaul value:  "
             << TrackerParameter.MediumMaterial.Data() << endl;
    } else {
        cout << "MediumMaterial     parameter set to user value: "
             << TrackerParameter.MediumMaterial.Data() << endl;
    }
    cout << endl;
    ...
    if (TrackerParameter.HitsInPlaneMaximum == -1) {
        cout << " ERROR: The field 'HitsInPlaneMaximum' is MANDATORY in config file (section "
                "tracker parameters)"
             << endl;
        gApplication->Terminate();
    }
    ...
    if (TrackerParameter.TracksMaximum > 0) {
        if (TrackerParameter.TracksFinder == 0) {
            if (TrackerParameter.SearchHitDistance == -1) {
                cout << " ERROR: The field 'SearchHitDistance' is MANDATORY in config file "
                        "(section tracker parameters)"
                     << endl;
                gApplication->Terminate();
            }
            if (TrackerParameter.PlanesForTrackMinimum == -1) {
                cout << " ERROR: The field 'PlanesForTrackMinimum' is MANDATORY in config file "
                        "(section tracker parameters)"
                     << endl;
                gApplication->Terminate();
            }
            if (TrackerParameter.HitsInPlaneTrackMaximum == -1) {
                cout << " ERROR: The field 'HitsInPlaneTrackMaximum' is MANDATORY in config file "
                        "(section tracker parameters)"
                     << endl;
                gApplication->Terminate();
            }
            ...
            if (TrackerParameter.SearchMoreHitDistance == -1) {
                cout << " ERROR: The field 'SearchMoreHitDistance' is MANDATORY in config file "
                        "(section tracker parameters) when using TracksFinder: 1"
                     << endl;
                gApplication->Terminate();
            }
        } else if (TrackerParameter.TracksFinder == 2) {
            if (TrackerParameter.TrackingPass == -1) {
                cout << " ERROR: The field 'TrackingPass' is MANDATORY in config file (section "
                        "tracker parameters) when using TracksFinder: 2"
                     << endl;
                gApplication->Terminate();
            }
            if (TrackerParameter.TrackingOrderLines == -1) {
                cout << " ERROR: The field 'TrackingOrderLines' is MANDATORY in config file "
                        "(section tracker parameters) when using TracksFinder: 2"
                     << endl;
                gApplication->Terminate();
            }
        }
...
void MimosaSimuSetup::ReadExperimentGeometryParameters() {
    ...
    if (!strcmp(fFieldName, "Inputs") || !strcmp(fFieldName, "LadderID")) {
        cout << "    -> No Experiment Geometrical Parameters to read (next item is '" << fFieldName
             << "')" << endl;
        return;
    }
    ...
    do {
        if (!strcmp(fFieldName, "GeometryName")) {
            ...
            cout << "      -> GeometryVersion:       " << IviGeometryParameter.GeometryVersion
                 << endl;
        } else if (!strcmp(fFieldName, "BeamOriginI")) {
            read_r3(IviGeometryParameter.BeamOrigin);
            cout << "      -> BeamOrigin [mm]:    I: " << IviGeometryParameter.BeamOrigin(0)
                 << endl;
            cout << "                             J: " << IviGeometryParameter.BeamOrigin(1)
                 << endl;
            cout << "                             K: " << IviGeometryParameter.BeamOrigin(2)
                 << endl;
            ...
            cout << "      -> BeamDisplayStrongBegin [mm]: I: "
                 << IviGeometryParameter.BeamDisplayStrongBegin(0) << endl;
            cout << "                                      J: "
                 << IviGeometryParameter.BeamDisplayStrongBegin(1) << endl;
            cout << "                                      K: "
                 << IviGeometryParameter.BeamDisplayStrongBegin(2) << endl;
        } else if (!strcmp(fFieldName, "BeamDisplayStrongStopI")) {
            read_r3(IviGeometryParameter.BeamDisplayStrongStop);
            cout << "      -> BeamDisplayStrongStop [mm]:  I: "
                 << IviGeometryParameter.BeamDisplayStrongStop(0) << endl;
            cout << "                                      J: "
                 << IviGeometryParameter.BeamDisplayStrongStop(1) << endl;
            cout << "                                      K: "
                 << IviGeometryParameter.BeamDisplayStrongStop(2) << endl;
        } else if (!strcmp(fFieldName, "BeamDisplayMediumBeginI")) {
            read_r3(IviGeometryParameter.BeamDisplayMediumBegin);
            cout << "      -> BeamDisplayMediumBegin [mm]: I: "
                 << IviGeometryParameter.BeamDisplayMediumBegin(0) << endl;
            cout << "                                      J: "
                 << IviGeometryParameter.BeamDisplayMediumBegin(1) << endl;
            cout << "                                      K: "
                 << IviGeometryParameter.BeamDisplayMediumBegin(2) << endl;
        } else if (!strcmp(fFieldName, "BeamDisplayMediumStopI")) {
            read_r3(IviGeometryParameter.BeamDisplayMediumStop);
            cout << "      -> BeamDisplayMediumStop [mm]:  I: "
                 << IviGeometryParameter.BeamDisplayMediumStop(0) << endl;
            cout << "                                      J: "
                 << IviGeometryParameter.BeamDisplayMediumStop(1) << endl;
            cout << "                                      K: "
                 << IviGeometryParameter.BeamDisplayMediumStop(2) << endl;
        } else if (!strcmp(fFieldName, "BeamDisplayLightBeginI")) {
            read_r3(IviGeometryParameter.BeamDisplayLightBegin);
            cout << "      -> BeamDisplayLightBegin [mm]:  I: "
                 << IviGeometryParameter.BeamDisplayLightBegin(0) << endl;
            cout << "                                      J: "
                 << IviGeometryParameter.BeamDisplayLightBegin(1) << endl;
            cout << "                                      K: "
                 << IviGeometryParameter.BeamDisplayLightBegin(2) << endl;
        } else if (!strcmp(fFieldName, "BeamDisplayLightStopI")) {
            read_r3(IviGeometryParameter.BeamDisplayLightStop);
            cout << "      -> BeamDisplayLightStop [mm]:   I: "
                 << IviGeometryParameter.BeamDisplayLightStop(0) << endl;
            cout << "                                      J: "
                 << IviGeometryParameter.BeamDisplayLightStop(1) << endl;
            cout << "                                      K: "
                 << IviGeometryParameter.BeamDisplayLightStop(2) << endl;
            ...
            cout << "      -> TargetSize [mm]  :  I: " << IviGeometryParameter.TargetSize(0)
                 << endl;
            cout << "                             J: " << IviGeometryParameter.TargetSize(1)
                 << endl;
            cout << "                             K: " << IviGeometryParameter.TargetSize(2)
                 << endl;
            ...
            cout << "      -> TargetAxis :           " << IviGeometryParameter.TargetAxis.Data()
                 << endl;
        } else if (!strcmp(fFieldName, "TargetCenterI")) {
            read_r3(IviGeometryParameter.TargetCenter);
            cout << "      -> TargetCenter [mm]:  I: " << IviGeometryParameter.TargetCenter(0)
                 << endl;
            cout << "                             J: " << IviGeometryParameter.TargetCenter(1)
                 << endl;
            cout << "                             K: " << IviGeometryParameter.TargetCenter(2)
                 << endl;
        } else if (!strcmp(fFieldName, "TrackerOriginI")) {
            read_r3(IviGeometryParameter.TrackerOrigin);
            cout << "      -> TrackerOrigin [mm]: I: " << IviGeometryParameter.TrackerOrigin(0)
                 << endl;
            cout << "                             J: " << IviGeometryParameter.TrackerOrigin(1)
                 << endl;
            cout << "                             K: " << IviGeometryParameter.TrackerOrigin(2)
                 << endl;
        } else if (!strcmp(fFieldName, "TrackerTiltI")) {
            read_r3(IviGeometryParameter.TrackerTilt);
            cout << "      -> TrackerTilt [deg]:  I: " << IviGeometryParameter.TrackerTilt(0)
                 << endl;
            cout << "                             J: " << IviGeometryParameter.TrackerTilt(1)
                 << endl;
            cout << "                             K: " << IviGeometryParameter.TrackerTilt(2)
                 << endl;
        } else if (!strcmp(fFieldName, "VertexingMethod")) {
            read_strings(IviGeometryParameter.VertexingMethod, IviGeometryParameter.tpsz);
            cout << "      -> VertexingMethod:       " << IviGeometryParameter.VertexingMethod
                 << endl;
        }
...
void MimosaSimuSetup::ReadLadderParameters(Int_t aLadderNumber) {
    ...
    if (aLadderNumber < 0 || TrackerParameter.Ladders <= aLadderNumber) {
        printf("WARNING in ReadLadderParameters: trying to add ladder number %d, outside the "
               "limits [1 - %d]\n  --> nothing done!\n",
    ...
    do {
        ...
        if (!strcmp(fFieldName, "LadderID")) {
            ...
            if (MimosaSimuSetupDebug)
                cout << " ID = " << pLadderParameter[aLadderNumber].LadderID << endl;
        } else if (!strcmp(fFieldName, "LadderName")) {
            read_strings(pLadderParameter[aLadderNumber].Name,
                         pLadderParameter[aLadderNumber].tpsz);
            if (MimosaSimuSetupDebug)
                cout << " Name = " << pLadderParameter[aLadderNumber].Name << endl;
        } else if (!strcmp(fFieldName, "Status")) {
            read_item(pLadderParameter[aLadderNumber].Status);
            /*if (MimosaSimuSetupDebug)*/ cout
                << " Status = " << pLadderParameter[aLadderNumber].Status << endl;
            ...
            if (MimosaSimuSetupDebug)
                cout << "  Position X = " << pLadderParameter[aLadderNumber].Position[0]
                     << ",  Y = " << pLadderParameter[aLadderNumber].Position[1]
                     << ", Z = " << pLadderParameter[aLadderNumber].Position[2] << " microns"
                     << endl;
        } else if (!strcmp(fFieldName, "LadderTiltZ")) {
            read_r3(pLadderParameter[aLadderNumber].Tilt);
            pLadderParameter[aLadderNumber].Tilt *=
                TMath::Pi() / 180.; // conversion degree to radian
            if (MimosaSimuSetupDebug)
                cout << "  Tilt X = " << pLadderParameter[aLadderNumber].Tilt[0]
                     << ",  Y = " << pLadderParameter[aLadderNumber].Tilt[1]
                     << ", Z = " << pLadderParameter[aLadderNumber].Tilt[2] << " rad" << endl;
        } else if (!strcmp(fFieldName, "NbOfPlanes")) {
            read_item(pLadderParameter[aLadderNumber].Planes);
            pLadderParameter[aLadderNumber].PlaneList =
                new Int_t[pLadderParameter[aLadderNumber].Planes];
            pLadderParameter[aLadderNumber].PlaneShift =
                new G4ThreeVector[pLadderParameter[aLadderNumber].Planes];
            pLadderParameter[aLadderNumber].PlaneTilt =
                new G4ThreeVector[pLadderParameter[aLadderNumber].Planes];
            if (MimosaSimuSetupDebug)
                cout << " Nb of planes = " << pLadderParameter[aLadderNumber].Planes << endl;
        } else if (!strcmp(fFieldName, "IncludedPlane")) {
            if (nbOfPlanesAssociated >= pLadderParameter[aLadderNumber].Planes) {
                printf("WARNING in ReadLadderParameters: trying to associate plane while maximum # "
                       "planes (%d) already reached --> nothing done!\n",
                ...
                if (MimosaSimuSetupDebug)
                    cout << "    Associated plane " << nbOfPlanesAssociated << " is "
                         << pLadderParameter[aLadderNumber].PlaneList[nbOfPlanesAssociated] << endl;
                ...
                if (!strcmp(fFieldName, "PlaneShiftU")) {
                    ...
                    pLadderParameter[aLadderNumber].PlaneShift[nbOfPlanesAssociated] *=
                        1000.; // conversion from mm to um
                    if (MimosaSimuSetupDebug)
                        cout << "    Plane shift U = "
                             << pLadderParameter[aLadderNumber].PlaneShift[nbOfPlanesAssociated][0]
                             << ",  V = "
                             << pLadderParameter[aLadderNumber].PlaneShift[nbOfPlanesAssociated][1]
                             << ", W = "
                             << pLadderParameter[aLadderNumber].PlaneShift[nbOfPlanesAssociated][2]
                             << " microns" << endl;
                } else {
                    printf(
                        "WARNING in ReadLadderParameters: expecting Plane %d Shift information!\n",
                ...
                if (!strcmp(fFieldName, "PlaneTiltU")) {
                    ...
                    pLadderParameter[aLadderNumber].PlaneTilt[nbOfPlanesAssociated] *=
                        M_PI / 180.; // conversion degree to radian
                    if (MimosaSimuSetupDebug)
                        cout << "    Plane Tilt U = "
                             << pLadderParameter[aLadderNumber].PlaneTilt[nbOfPlanesAssociated][0]
                             << ",  V = "
                             << pLadderParameter[aLadderNumber].PlaneTilt[nbOfPlanesAssociated][1]
                             << ", W = "
                             << pLadderParameter[aLadderNumber].PlaneTilt[nbOfPlanesAssociated][2]
                             << " [rad]" << endl;
                } else {
                    printf(
                        "WARNING in ReadLadderParameters: expecting Plane %d Tilt information!\n",
                        nbOfPlanesAssociated);
                }
    ...
    if (nbOfPlanesAssociated < pLadderParameter[aLadderNumber].Planes) {
        printf("WARNING in ReadLadderParameters: only %d planes have been associated to ladder %d "
               "while %d were expected!\n",
               nbOfPlanesAssociated, pLadderParameter[aLadderNumber].LadderID,
               pLadderParameter[aLadderNumber].Planes);
    }
    ...
    /*
    if(DSetupDebug) {
      printf("Parameters for ladder %d of type %s:\n", pLadderParameter[aLadderNumber].LadderID,
    pLadderParameter[aLadderNumber].Name); printf("  status = %d\n",
    pLadderParameter[aLadderNumber].Status); printf("  contains %d ladders:\n",
    pLadderParameter[aLadderNumber].Planes); for ( Int_t iplane=0;
    iplane<pLadderParameter[aLadderNumber].Planes; iplane++) { printf("    plane %d -> absolute nb =
    %d\n", iplane, pLadderParameter[aLadderNumber].PlaneList[iplane]); printf ("   relative tilt :
    %.2f %.2f %.2f \n" , pLadderParameter[aLadderNumber].PlaneTilt[iplane](0) ,
    pLadderParameter[aLadderNumber].PlaneTilt[iplane](1) ,
    pLadderParameter[aLadderNumber].PlaneTilt[iplane](2)); printf ("   relative shift : %.2f %.2f
...
void MimosaSimuSetup::ReadPlaneParameters(Int_t aPlaneNumber) {
    ...
    if (aPlaneNumber < 0 || TrackerParameter.Planes <= aPlaneNumber) {
        printf("WARNING in ReadPlaneParameters: trying to add plane number %d, outside the limits "
               "[1 - %d]\n  --> nothing done!\n",
    ...
    for (int kkk = 0; kkk < 20; kkk++)
        pPlaneParameter[aPlaneNumber].EtaCoefficient[kkk] = -999.0;
    ...
    for (int kkk = 0; kkk < 20; kkk++)
        pPlaneParameter[aPlaneNumber].KappaCoefficient[kkk] = -999.0;
    ...
    for (int kkk = 0; kkk < 20; kkk++)
        pPlaneParameter[aPlaneNumber].GammaCoefficient[kkk] = -999.0;
    ...
    for (int kkk = 0; kkk < 20; kkk++)
        pPlaneParameter[aPlaneNumber].NoisyStripsIndex[kkk] = -1;
    ...
    for (int kkk = 0; kkk < fMaxDigitalThresholds; kkk++)
        pPlaneParameter[aPlaneNumber].DigitizeThresholds[kkk] = -999.0;
    ...
    pPlaneParameter[aPlaneNumber].ClusterLimit(0) = pPlaneParameter[aPlaneNumber].ClusterLimit(1) =
        pPlaneParameter[aPlaneNumber].ClusterLimit(3) = -1;
    for (int i = 0; i < 7; i++) {
    ...
    do {
        ...
        if (!strcmp(fFieldName, "Inputs")) {
            ...
            if (MimosaSimuSetupDebug)
                cout << "   " << pPlaneParameter[aPlaneNumber].Inputs << " inputs:" << endl;
            ...
            for (Int_t tl = 0; tl < pPlaneParameter[aPlaneNumber].Inputs; tl++) {
                ...
                if (MimosaSimuSetupDebug)
                    cout << "    input " << tl << " of type "
                         << pPlaneParameter[aPlaneNumber].ModuleType[tl] << " number "
                         << pPlaneParameter[aPlaneNumber].ModuleNumber[tl] << " input "
                         << pPlaneParameter[aPlaneNumber].InputNumber[tl] << " channel "
                         << pPlaneParameter[aPlaneNumber].ChannelNumber[tl] << " channels "
                         << pPlaneParameter[aPlaneNumber].Channels[tl] << endl;
            }
            ...
            pPlaneParameter[aPlaneNumber].InitialPedestal =
                pPlaneParameter[aPlaneNumber]
                    .InitialNoise; // added to initialize when only InitialNoise is specified, JB
                                   // 2014/01/07
        } else if (!strcmp(fFieldName, "CacheSize")) {
            read_item(pPlaneParameter[aPlaneNumber].CacheSize);
            if (MimosaSimuSetupDebug)
                printf("  CacheSize : %d     InitialPedestal : %d      InitialNoise : %d\n",
            ...
            if (pPlaneParameter[aPlaneNumber].IfDigitize > 0) { // if digitization required
                if (MimosaSimuSetupDebug)
                    printf("  Digitization emulation required over %d thresholds:\n",
                ...
                if (!strcmp(fFieldName, "DigitizeThresholds")) { // check there are the thresholds
                    for (Int_t k = 0; k < pPlaneParameter[aPlaneNumber].IfDigitize; k++) {
                        ...
                        if (MimosaSimuSetupDebug)
                            printf("  thre[%d] = %d\n", k,
                                   pPlaneParameter[aPlaneNumber].DigitizeThresholds[k]);
                    }
                } else { // if the thresholds are missing
                    Error("MimosaSimuSetup",
                          "In the configuration file, the field 'DigitizeThresholds' is expected "
                          "right after the 'IfDigitize' one, with %d thresholds !",
                          pPlaneParameter[aPlaneNumber].IfDigitize);
                }
            ...
            if (MimosaSimuSetupDebug)
                cout << "  Position X = " << pPlaneParameter[aPlaneNumber].Position(0)
                     << ",  Y = " << pPlaneParameter[aPlaneNumber].Position(1)
                     << ", Z = " << pPlaneParameter[aPlaneNumber].Position(2) << " microns" << endl;
            ...
            if (MimosaSimuSetupDebug)
                cout << "  AlignementU " << pPlaneParameter[aPlaneNumber].AlignmentU
                     << ",  AlignementV " << pPlaneParameter[aPlaneNumber].AlignmentV
                     << ",  AlignementTilt [deg] " << pPlaneParameter[aPlaneNumber].AlignmentTilt
                     << endl;
        } else if (!strcmp(fFieldName, "SizeU")) {
            read_r3(pPlaneParameter[aPlaneNumber].Size);
            pPlaneParameter[aPlaneNumber].Size *= 1000;
            if (MimosaSimuSetupDebug)
                printf("  Size : %f %f %f \n", pPlaneParameter[aPlaneNumber].Size[0],
                       pPlaneParameter[aPlaneNumber].Size[1],
                       pPlaneParameter[aPlaneNumber].Size[2]);
            printf("WARNING: you specify a Size for your strips, which is now ignored.\n The "
                   "sensitive area is now computed from the Pitch and the nb of strips.\n");
        } else if (!strcmp(fFieldName, "StripsU")) {
            read_r3(pPlaneParameter[aPlaneNumber].Strips);
            if (MimosaSimuSetupDebug)
                printf("  #strips : %f %f %f \n", pPlaneParameter[aPlaneNumber].Strips[0],
                       pPlaneParameter[aPlaneNumber].Strips[1],
                       pPlaneParameter[aPlaneNumber].Strips[2]);
        } else if (!strcmp(fFieldName, "PitchU")) {
            read_r3(pPlaneParameter[aPlaneNumber].Pitch);
            pPlaneParameter[aPlaneNumber].Pitch *= 1000;
            if (MimosaSimuSetupDebug)
                printf("  strip pitch : %f %f %f \n", pPlaneParameter[aPlaneNumber].Pitch[0],
                       pPlaneParameter[aPlaneNumber].Pitch[1],
                       pPlaneParameter[aPlaneNumber].Pitch[2]);
            ...
            if (MimosaSimuSetupDebug)
                printf("  strip size : %f %f %f \n", pPlaneParameter[aPlaneNumber].StripSize[0],
                       pPlaneParameter[aPlaneNumber].StripSize[1],
                       pPlaneParameter[aPlaneNumber].StripSize[2]);
        } else if (!strcmp(fFieldName, "Mapping")) {
            read_item(pPlaneParameter[aPlaneNumber].Mapping);
            if (MimosaSimuSetupDebug)
                cout << "  Mapping = " << pPlaneParameter[aPlaneNumber].Mapping << endl;
        } else if (!strcmp(fFieldName, "ThreshNeighbourSN") ||
                   !strcmp(fFieldName,
                           "ThresNeighbourSN")) { // additional condition (typo), JB 2013/08/19
            ...
            if (MimosaSimuSetupDebug)
                cout << "  Threshold SNR cuts, neighbour = "
                     << pPlaneParameter[aPlaneNumber].ThreshNeighbourSN
                     << " seed = " << pPlaneParameter[aPlaneNumber].ThreshSeedSN << endl;
            ...
            if (MimosaSimuSetupDebug)
                cout << "  Status of plane = " << pPlaneParameter[aPlaneNumber].Status << endl;
        } else if (!strcmp(fFieldName, "ParentLadder")) {
            read_item(pPlaneParameter[aPlaneNumber].ParentLadderID);
            if (MimosaSimuSetupDebug)
                cout << "  Parent ladder ID = " << pPlaneParameter[aPlaneNumber].ParentLadderID
                     << endl;
        ...
        // Plane digitization parameters
            ...
            if (TheRegion.R_col[0] == -1 || TheRegion.R_col[1] == -1 || TheRegion.R_lin[0] == -1 ||
                TheRegion.R_lin[1] == -1) {
                cout << endl;
                cout << "When specifying a region need to give 4 parameters. col_min, col_max, "
                        "line_min and line_max. Check your inputs. Exiting now!!!"
                     << endl;
                cout << endl;
                assert(false);
            }
        ...
        } else if (!strcmp(fFieldName, "Digitizer") ||
                   !strcmp(fFieldName, "Digitize")) { // Digitization parameters, JB 2013/08/29
            read_item(pPlaneParameter[aPlaneNumber].IfDigitize);
            if (pPlaneParameter[aPlaneNumber].IfDigitize > fMaxDigitalThresholds) {
                Warning("ReadPlaneParameters",
                        "Nb of digits specified %d, exceeds limit %d --> set back to limit",
                        pPlaneParameter[aPlaneNumber].IfDigitize, fMaxDigitalThresholds);
                pPlaneParameter[aPlaneNumber].IfDigitize = fMaxDigitalThresholds;
            }
            if (MimosaSimuSetupDebug)
                cout << "  Digitize over " << pPlaneParameter[aPlaneNumber].IfDigitize
                     << " thresholds:" << endl;
            for (Int_t ithre = 0; ithre < pPlaneParameter[aPlaneNumber].IfDigitize; ithre++) {
                ...
                if (MimosaSimuSetupDebug)
                    cout << " " << pPlaneParameter[aPlaneNumber].DigitizeThresholds[ithre] << endl;
            }
            if (MimosaSimuSetupDebug) cout << endl;
        } // end Digitization parameters

        else if (!strcmp(fFieldName, "Deformed") ||
                 !strcmp(fFieldName, "deformed")) { // JB 2014/04/21
            read_item(pPlaneParameter[aPlaneNumber].IfDeformed);
            if (MimosaSimuSetupDebug && pPlaneParameter[aPlaneNumber].IfDeformed > 0)
                cout << "  Deforming plane surface according to parameters:" << endl;
        } else if (!strcmp(fFieldName, "LegendreU") ||
                   !strcmp(fFieldName, "legendreU")) { // JB 2014/04/21
            Int_t i = 0;
            if (MimosaSimuSetupDebug) cout << "     along U:";
            while (i < 7) {
                read_item(pPlaneParameter[aPlaneNumber].CoeffLegendreU[i]);
                if (MimosaSimuSetupDebug)
                    cout << " " << pPlaneParameter[aPlaneNumber].CoeffLegendreU[i];
                nextItem(':');
                i++;
            }
            if (MimosaSimuSetupDebug) cout << endl;
        } // end U Deformation parameters
        else if (!strcmp(fFieldName, "LegendreV") ||
                 !strcmp(fFieldName, "legendreV")) { // JB 2014/04/21
            Int_t i = 0;
            if (MimosaSimuSetupDebug) cout << "     along V:";
            while (i < 7) {
                read_item(pPlaneParameter[aPlaneNumber].CoeffLegendreV[i]);
                if (MimosaSimuSetupDebug)
                    cout << " " << pPlaneParameter[aPlaneNumber].CoeffLegendreV[i] << endl;
                nextItem(':');
                i++;
            }
            if (MimosaSimuSetupDebug) cout << endl;
        } // end V Deformation parameters
        else if (!strcmp(fFieldName, "PlaneTimeLimit") ||
                 !strcmp(fFieldName, "planetimelimit")) { // JB 2015/05/26
            read_item(pPlaneParameter[aPlaneNumber].TimeLimit);
        } else {
            if (strcmp(fFieldName, "Inputs") && strcmp(fFieldName, "LadderID") &&
                strcmp(fFieldName, "FileHeaderSize") && strcmp(fFieldName, "ModuleTypes") &&
    ...
    if (pPlaneParameter[aPlaneNumber].HitFinder == 2) {
        if (pPlaneParameter[aPlaneNumber].ClusterLimitRadius == -1) {
            cout << " ERROR: The field 'ClusterLimitRadius' is MANDATORY in config file when using "
                    "HitFinder = 2 (section planes parameters)"
                 << endl;
            gApplication->Terminate();
        }
    }
    if (pPlaneParameter[aPlaneNumber].HitFinder != 2) {
        if (pPlaneParameter[aPlaneNumber].ClusterLimit(0) == -1 ||
            pPlaneParameter[aPlaneNumber].ClusterLimit(1) == -1) {
            cout << " ERROR: The field 'ClusterLimit' is MANDATORY in config file when using "
                    "HitFinder != 2 (section planes parameters)"
                 << endl;
            gApplication->Terminate();
        }
    ...
    if (_ResolutionFileList.size() > 0) {
        ...
        // NOTE: currently using histogram with residues (huCGwidth_vs_Mult and hvCGwidth_vs_Mult)
        // instead of resolution. Will correction this later
        ...
        if (_ResolutionFileList.size() == 1) {
            for (int ireg = 0; ireg < int(_ResolutionFileList.size()); ireg++) {
                ...
                if (ResolutionVsMultFile != NULL) {
                    ...
                    if (hMultiplicity == NULL) {
                        cout << endl;
                        cout << "Histogram with name " << TheHistoName.Data()
                             << " is not found in ROOT file " << _ResolutionFileList[ireg].Data()
                             << " for plane " << aPlaneNumber << " and region " << ireg + 1
                             << "doesn't exits. Check your inputs. Exiting now!!!" << endl;
                        cout << endl;
                        assert(false);
                    }
                    ...
                    for (int imult = 0; imult < hResolutionU->GetXaxis()->GetNbins(); imult++) {
                        AResolutionVsMult.ResolutionU.push_back(
                            hResolutionU->GetBinContent(imult + 1));
                        AResolutionVsMult.ResolutionV.push_back(
                            hResolutionV->GetBinContent(imult + 1));
                    }
                    for (int imult = 0; imult < hResolutionU->GetXaxis()->GetNbins(); imult++) {
                        if (imult + 1 < hResolutionU->GetXaxis()->GetNbins()) {
                            double prob =
                                hMultiplicity->GetBinContent(hMultiplicity->FindBin(imult + 1));
                    ...
                    pPlaneParameter[aPlaneNumber].PlanePerformancesList.push_back(
                        AResolutionVsMult);
                    ...
                    cout << endl;
                    cout << "Resolution vs multiplicity ROOT file "
                         << _ResolutionFileList[ireg].Data() << " for plane " << aPlaneNumber
                         << " and region " << ireg + 1
                         << "doesn't exits. Check your inputs. Exiting now!!!" << endl;
                    cout << endl;
                    assert(false);
            ...
            // Check that the regions defined agree with the sensor limits and that they not overlap
            // with each others
            bool RegionsOverlap = false;
            for (int ireg = 0; ireg < int(_RegionList.size()); ireg++) {
                for (int jreg = ireg + 1; jreg < int(_RegionList.size()); jreg++) {
                    int Corners[4][2];
                    Corners[0][0] = _RegionList[jreg].R_col[0];
                    Corners[0][1] = _RegionList[jreg].R_lin[0];
                    Corners[1][0] = _RegionList[jreg].R_col[1];
                    Corners[1][1] = _RegionList[jreg].R_lin[0];
                    Corners[2][0] = _RegionList[jreg].R_col[0];
                    Corners[2][1] = _RegionList[jreg].R_lin[1];
                    Corners[3][0] = _RegionList[jreg].R_col[1];
                    Corners[3][1] = _RegionList[jreg].R_lin[1];

                    for (int icorners = 0; icorners < 4; icorners++) {
                        if ((Corners[icorners][0] >= _RegionList[ireg].R_col[0] &&
                             Corners[icorners][0] <= _RegionList[ireg].R_col[1]) &&
                            (Corners[icorners][1] >= _RegionList[ireg].R_lin[0] &&
                             Corners[icorners][1] <= _RegionList[ireg].R_lin[1])) {
                            RegionsOverlap = true;
                            cout << "region " << ireg + 1 << " overlaps with region " << jreg + 1
                                 << endl;
                            break;
                        }
                    }
            ...
            if (RegionsOverlap) {
                cout << endl;
                cout << "At least a couple of the regions defined for plane " << aPlaneNumber
                     << " overlap. Check you inputs. Exiting now!!!!" << endl;
                cout << endl;
                assert(false);
            }
            ...
            for (int ireg = 0; ireg < int(_RegionList.size()); ireg++) {
                if (_RegionList[ireg].R_col[0] < 0 ||
                    ...
                    BadRegion = true;
                    cout << "Region " << ireg + 1
                         << " has limits outside sensor limits!. Region col = ("
                         << _RegionList[ireg].R_col[0] << "," << _RegionList[ireg].R_col[1]
                         << "), lin = (" << _RegionList[ireg].R_lin[0] << ","
                         << _RegionList[ireg].R_lin[1] << "). Sensor col = (" << 0 << ","
                         << pPlaneParameter[aPlaneNumber].Strips(0) - 1 << "), lin = (" << 0 << ","
                         << pPlaneParameter[aPlaneNumber].Strips(1) - 1 << ")" << endl;
                    break;
                }
            }
            if (BadRegion) {
                cout << endl;
                cout
                    << "At least a of the regions defined for Plane " << aPlaneNumber + 1
                    << " overlap is outside of the sensor limits. Check you inputs. Exiting now!!!!"
                    << endl;
                cout << endl;
                assert(false);
            }
            for (int ireg = 0; ireg < int(_RegionList.size()); ireg++) {
                for (int jreg = ireg + 1; jreg < int(_RegionList.size()); jreg++) {
                    if (_ResolutionFileList[ireg] == _ResolutionFileList[jreg]) {
                        cout << "WARNING: files for regions " << ireg + 1 << " and " << jreg + 1
                             << " for Plane " << aPlaneNumber + 1 << " are the same!!!." << endl;
                    }
                }
            }
        ...
        if (_ResolutionUList.size() != _ResolutionVList.size()) {
            cout << endl;
            cout << "Need to specify both the same number of PlaneResolutionU and PlaneResolutionV "
                    "parameters for Plane "
                 << aPlaneNumber + 1 << ". Check your inputs. Exiting now!!!" << endl;
            cout << endl;
            assert(false);
        }
        ...
        if (_ResolutionUList.size() == 1 && _ResolutionVList.size() == 1) {
            ...
            if (_ResolutionUList.size() != _RegionList.size()) {
                cout << endl;
                cout << "Number of PlaneResolutionU/PlaneResolutionV parameters given ("
                     << _ResolutionUList.size() << ") is different from number of Regions given ("
                     << _RegionList.size() << "). Check you inputs. Exiting now!!!!" << endl;
                cout << endl;
                assert(false);
            }
        ...
        if (_ResolutionList.size() == 1) {
            ...
            if (RegionsOverlap) {
                cout << endl;
                cout << "At least a couple of the regions defined for Plane " << aPlaneNumber + 1
                     << " overlap. Check you inputs. Exiting now!!!!" << endl;
                cout << endl;
                assert(false);
            }
    ...
    if (pPlaneParameter[aPlaneNumber].PlanePerformancesList.size() == 1) {
        cout << "Using fake rate of "
             << pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].FakeRate
             << " for the whole matrix" << endl;

        // No region defined:
        if (pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].ResolutionU.size() == 0) {
            // No multiplicity dependent resolution defined
            if (pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].GlobalPlaneResolutionU ==
                -1) {
                ...
                if (pPlaneParameter[aPlaneNumber].UsingTrackerResolution) {
                    ...
                    cout << "Using global TrakerResolution for PlaneResolution for Plane "
                         << aPlaneNumber + 1 << ". Resolution = "
                         << pPlaneParameter[aPlaneNumber]
                                .PlanePerformancesList[0]
                                .GlobalPlaneResolution
                         << "um" << endl;
                    cout << "Using the same resolution for U and V directions!!!" << endl;
                } else {
                    // Using user defined global resolution
                    cout << "Using user defined global PlaneResolution for Plane "
                         << aPlaneNumber + 1 << ". Resolution = "
                         << pPlaneParameter[aPlaneNumber]
                                .PlanePerformancesList[0]
                                .GlobalPlaneResolution
                ...
                cout << "Using different PlaneResolutions for Plane " << aPlaneNumber + 1
                     << " for U and V directions." << endl;
                cout
                    << "ResolutionU = "
                    << pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].GlobalPlaneResolutionU
                    << "um" << endl;
                cout
                    << "ResolutionV = "
                    << pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].GlobalPlaneResolutionV
                    << "um" << endl;
            }
        } else {
            // Using multiplicity dependent resolutions for U and V
            cout << "Multiplicity dependent resolution for Plane" << aPlaneNumber + 1 << ":"
                 << endl;
            for (int imult = 0;
                 imult <
                 int(pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].ResolutionU.size());
                 imult++) {
                cout
                    << "(ResolutionU,ResolutionV) = ("
                    << pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].ResolutionU[imult]
                    << ","
                    << pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].ResolutionV[imult]
                    << ") um (prob = "
                    << pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].MultProb[imult] *
                           100.0
                    << " %, prob_cumul = "
                    << pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].MultProbCumul[imult] *
                           100.0
                    << " %) for ";
                if (imult <
                    int(pPlaneParameter[aPlaneNumber].PlanePerformancesList[0].ResolutionU.size() -
        ...
        cout << "Found " << pPlaneParameter[aPlaneNumber].PlanePerformancesList.size()
             << " regions for Plane " << aPlaneNumber + 1 << "." << endl;
        for (int ireg = 0; ireg < int(pPlaneParameter[aPlaneNumber].PlanePerformancesList.size());
             ireg++) {
            cout << "Region " << ireg + 1 << ": col = ("
                 << pPlaneParameter[aPlaneNumber].PlanePerformancesList[ireg].Region.R_col[0] << ","
                 << pPlaneParameter[aPlaneNumber].PlanePerformancesList[ireg].Region.R_col[1]
                 << "), lin = ("
                 ...
                 << ")."
                 << " Estimated fake rate = "
                 << pPlaneParameter[aPlaneNumber].PlanePerformancesList[ireg].FakeRate << endl;
            if (pPlaneParameter[aPlaneNumber].PlanePerformancesList[ireg].ResolutionU.size() == 0) {
                // No multiplicity dependent resolution defined
                if (pPlaneParameter[aPlaneNumber]
                        .PlanePerformancesList[ireg]
                        .GlobalPlaneResolutionU == -1) {
                    // Using same resolution for U and V
                    if (pPlaneParameter[aPlaneNumber].UsingTrackerResolution) {
                        // Using same resolution as the one defined on tracker block
                        cout << "Using global TrakerResolution for PlaneResolution for Plane "
                             << aPlaneNumber + 1 << " and Region " << ireg + 1 << ". Resolution = "
                             << pPlaneParameter[aPlaneNumber]
                                    .PlanePerformancesList[ireg]
                                    .GlobalPlaneResolution
                             << "um" << endl;
                ...
                // Using multiplicity dependent resolutions for U and V
                for (int imult = 0; imult < int(pPlaneParameter[aPlaneNumber]
                                                    .PlanePerformancesList[ireg]
                                                    .ResolutionU.size());
                     imult++) {
                    cout << "(ResolutionU,ResolutionV) = ("
                         ...
                         << pPlaneParameter[aPlaneNumber]
                                ...
                                100.0
                         << " %) for ";
                    if (imult < int(pPlaneParameter[aPlaneNumber]
                                        .PlanePerformancesList[ireg]
                                        .ResolutionU.size() -
                                    1)) {
                        cout << " mult = " << imult + 1;
    ...
    if (pPlaneParameter[aPlaneNumber].PlaneThickness < 0) {
        ...
        cout << "No specification of PlaneThickness. Use default value of "
             << pPlaneParameter[aPlaneNumber].PlaneThickness
             << "um. (Insignificant multiple scattering)" << endl;
    } else
        cout << "PlaneThickness set to user value: " << pPlaneParameter[aPlaneNumber].PlaneThickness
             << "um." << endl;

    if (pPlaneParameter[aPlaneNumber].PlaneMaterial == TString("")) {
        pPlaneParameter[aPlaneNumber].PlaneMaterial = TString("silicon");
        cout << "No specification of PlaneMaterial.  Use default value of "
             << pPlaneParameter[aPlaneNumber].PlaneMaterial.Data() << "." << endl;
    } else {
        cout << "PlaneMaterial  set to user value: "
             << pPlaneParameter[aPlaneNumber].PlaneMaterial.Data() << endl;
    }

    if ((pPlaneParameter[aPlaneNumber].PlaneMetalThickness >= 0 &&
         pPlaneParameter[aPlaneNumber].PlaneMetalThickness <= 1) &&
        (pPlaneParameter[aPlaneNumber].PlaneEpiThickness >= 0 &&
         pPlaneParameter[aPlaneNumber].PlaneEpiThickness <= 1)) {
        if (pPlaneParameter[aPlaneNumber].PlaneMetalThickness +
                pPlaneParameter[aPlaneNumber].PlaneEpiThickness >
            1) {
            cout << "WARNING : Plane " << aPlaneNumber + 1
                 << " parameters PlaneMetalThickness and PlaneEpiThickness have values which sum "
                    "exceeds 1. Setting them to default values 0 and 1, respectively!"
                 << endl;
            pPlaneParameter[aPlaneNumber].PlaneMetalThickness = 0.0;
            pPlaneParameter[aPlaneNumber].PlaneEpiThickness   = 1.0;
        } else {
            cout << "PlaneMetalThickness set to value: "
                 << pPlaneParameter[aPlaneNumber].PlaneMetalThickness * 100
                 << " % of total thickness ("
                 << pPlaneParameter[aPlaneNumber].PlaneMetalThickness *
                        pPlaneParameter[aPlaneNumber].PlaneThickness
                 << " um)" << endl;
            cout << "PlaneEpiThickness   set to value: "
                 << pPlaneParameter[aPlaneNumber].PlaneEpiThickness * 100
                 << " % of total thickness ("
                 << pPlaneParameter[aPlaneNumber].PlaneEpiThickness *
                        pPlaneParameter[aPlaneNumber].PlaneThickness
                 << " um)" << endl;
        ...
        cout << "WARNING : Plane " << aPlaneNumber + 1
             << " parameters PlaneMetalThickness and/or PlaneEpiThickness have values outside "
                "valid range (0,1). Setting them to default values 0 and 1, respectively!"
             << endl;
        pPlaneParameter[aPlaneNumber].PlaneMetalThickness = 0.0;
        pPlaneParameter[aPlaneNumber].PlaneEpiThickness   = 1.0;
    }
    ...
    if (pPlaneParameter[aPlaneNumber].Readout > 0)
        ListOfSensitivePlanes.push_back(aPlaneNumber + 1);
...
void MimosaSimuSetup::ReadDAQParameters() {
    ...
    AcqParameter.EventBuildingMode =
        1; // SS 2011.11.14. Setting EventBuildingMode=1 by default. It can be changed during
           // initialisation of the session or from the config file
    ...
    do {
        ...
        } else if (!strcmp(fFieldName, "FileHeaderLine") ||
                   !strcmp(fFieldName, "FileHeaderLine[d]")) {
...
void MimosaSimuSetup::ReadDAQBoardParameters(Int_t aBoardNumber) {
    ...
    if (aBoardNumber < 0 || AcqParameter.ModuleTypes <= aBoardNumber) {
        printf("WARNING in ReadDAQBoardParameters: trying to add a board type %d, outside the "
               "limits [1 - %d]\n  --> nothing done!\n",
               aBoardNumber + 1, AcqParameter.ModuleTypes);
        return;
    } else {
        cout << endl
             << " - Reading Parameters of the Data Acquisition board type " << aBoardNumber + 1
             << endl;
    }

    // Some default values
    pAcqModuleParameter[aBoardNumber].EventBuildingBoardMode =
        AcqParameter.EventBuildingMode; // default, JB 2013/06/22
    ...
    do {
        ...
        if (!strcmp(fFieldName, "Name")) {
            read_strings(pAcqModuleParameter[aBoardNumber].Name,
                         pAcqModuleParameter[aBoardNumber].tpsz);
        } else if (!strcmp(fFieldName, "Type")) {
            read_item(pAcqModuleParameter[aBoardNumber].Type);
            pAcqModuleParameter[aBoardNumber].EventBuildingBoardMode =
                pAcqModuleParameter[aBoardNumber].Type % 10; // JB 2013/06/22
        ...
        } else if (!strcmp(fFieldName, "PixelShift") &&
                   strcmp(fFieldName, "PixelShiftMod")) { // JB,CB,PLR 2015/03/24
            ...
            for (Int_t iMod = 0; iMod < pAcqModuleParameter[aBoardNumber].Devices; iMod++) {
                if (pAcqModuleParameter[aBoardNumber].PixelShiftMod[iMod] == -1) {
                    pAcqModuleParameter[aBoardNumber].PixelShiftMod[iMod] =
                        pAcqModuleParameter[aBoardNumber].PixelShift;
                }
            }
            ...
            for (Int_t iMod = 0; iMod < pAcqModuleParameter[aBoardNumber].Devices; iMod++) {
                pAcqModuleParameter[aBoardNumber].DeviceDataFile[iMod] =
                    new Char_t[pAcqModuleParameter[aBoardNumber].tpsz];
                pAcqModuleParameter[aBoardNumber].PixelShiftMod[iMod] = 3; // default value
                if (iMod > 0) nextField();
                if (strstr(fFieldName, "DataFile")) {
                    read_strings(pAcqModuleParameter[aBoardNumber].DeviceDataFile[iMod],
                                 pAcqModuleParameter[aBoardNumber].tpsz);
                } else {
                    printf("WARNING in ReadDAQBoardParameters: field %s found while 'DataFile%d' "
                           "expected!\n    -> Board %d of type %d lacks rawdata connection.",
                           fFieldName, iMod + 1, iMod + 1, aBoardNumber);
                }
            }
        } else if (!strcmp(fFieldName, "PixelShiftMod") ||
                   !strcmp(fFieldName, "PixelShiftMod1")) { // JB,CB,PLR 2015/03/24
            ...
            for (Int_t iMod = 0; iMod < pAcqModuleParameter[aBoardNumber].Devices; iMod++) {
                ...
                if (strstr(fFieldName, "PixelShiftMod")) {
                    ...
                    printf("WARNING in ReadDAQBoardParameters: field %s found while "
                           "'PixelShiftMod%d' expected!\n    -> Board %d of type %d lacks kShift.",
                           fFieldName, iMod + 1, iMod + 1, aBoardNumber);
                }
            }
        } else {
            if (strcmp(fFieldName, "StatisticCells") && strcmp(fFieldName, "CmsNoiseCut") &&
                strcmp(fFieldName, "MaxNbOfHits") && strcmp(fFieldName, "Name") &&
                strcmp(fFieldName, "AnalysisGoal")) {
                cout << "WARNING : parameter '" << fFieldName
                     << "' in config file is not understood !" << endl;
                getRidOfLine();
            }
    ...
    } while (strcmp(fFieldName, "StatisticCells") && strcmp(fFieldName, "CmsNoiseCut") &&
             strcmp(fFieldName, "MaxNbOfHits") && strcmp(fFieldName, "Name") &&
             strcmp(fFieldName, "AnalysisGoal"));
    ...
    for (Int_t aInp = inputCounter + 1; aInp < pAcqModuleParameter[aBoardNumber].Inputs; aInp++) {
        ...
        pAcqModuleParameter[aBoardNumber].Channels[aInp] =
            pAcqModuleParameter[aBoardNumber].Channels[inputCounter];
        pAcqModuleParameter[aBoardNumber].Bits[aInp] =
            pAcqModuleParameter[aBoardNumber].Bits[inputCounter];
        pAcqModuleParameter[aBoardNumber].SigBits[aInp] =
            pAcqModuleParameter[aBoardNumber].SigBits[inputCounter];
        pAcqModuleParameter[aBoardNumber].NbOfFramesPerChannel[aInp] =
            pAcqModuleParameter[aBoardNumber].NbOfFramesPerChannel[inputCounter];
    }

    if (MimosaSimuSetupDebug) {
        cout << "  Using " << pAcqModuleParameter[aBoardNumber].Devices << " "
             << pAcqModuleParameter[aBoardNumber].Name << " with "
             << pAcqModuleParameter[aBoardNumber].Inputs << " inputs and "
             << pAcqModuleParameter[aBoardNumber].Bits << " bits(signif. "
             << pAcqModuleParameter[aBoardNumber].SigBits << ") and data files ";
        for (Int_t iMod = 0; iMod < pAcqModuleParameter[aBoardNumber].Devices; iMod++) {
            cout << " (" << iMod + 1 << ")"
                 << pAcqModuleParameter[aBoardNumber].DeviceDataFile[iMod];
        }
...
void MimosaSimuSetup::ReadAnalysisParameters() {
    ...
    do {
        ...
        if (!strcmp(fFieldName, "StatisticCells")) {
            ...
            read_item(temp);
            if (temp == 1)
                AnalysisParameter.SavePlots = true;
            else
                AnalysisParameter.SavePlots = false;
            ...
            read_item(temp);
            if (temp == 1)
                AnalysisParameter.DoTelResolutionMC = true;
            else
                AnalysisParameter.DoTelResolutionMC = false;
            if (AnalysisParameter.DoTelResolutionMC) {
                cout << endl;
                cout << "Analysis Parameters:: Doing MC generation for evaluation of Telescope "
                        "resolution at DUT!"
                     << endl;
                cout << endl;
            }
            ...
            read_item(temp);
            if (temp == 1)
                AnalysisParameter.MCDoDisplay = true;
            else
                AnalysisParameter.MCDoDisplay = false;
            ...
            if (temp == 1)
                ...
                AnalysisParameter.DoGaussianMS = false;
        } else if (!strcmp(fFieldName, "TrackChi2Limit")) {
            read_item(AnalysisParameter.TrackChi2Limit);
        ...
        } else if (!strcmp(fFieldName, "Submatrices") ||
                   !strcmp(fFieldName, "Subamtrices")) { // note the second test to take into
                                                         // account typo propagated years ago !
    ...
    if (MimosaSimuSetupDebug) {
        ...
        if (AnalysisParameter.MaxTracksExGeom > -1)
            cout << "  Max #tracks allowed in geomatrix " << AnalysisParameter.ExGeomatrix << " is "
                 << AnalysisParameter.MaxTracksExGeom << endl;
...
void MimosaSimuSetup::ReadSubmatrixParameters(Int_t aSubmatrixNumber) {
    ...
    do {
        if (!strcmp(fFieldName, "PixelSizeU")) {
            ...
            if (AnalysisParameter.Geomatrices[aSubmatrixNumber] == fMaxGeomatrices) {
                printf("WARNING in ReadSubmatrixParameters: maximum nb of Geomatrix reached, "
                       "taking no more!\n");
            } else {
                read_item(AnalysisParameter.Umin[aSubmatrixNumber]
                                                [AnalysisParameter.Geomatrices[aSubmatrixNumber]]);
                nextItem(':');
                read_item(AnalysisParameter.Umax[aSubmatrixNumber]
                                                [AnalysisParameter.Geomatrices[aSubmatrixNumber]]);
                nextItem(':');
                read_item(AnalysisParameter.Vmin[aSubmatrixNumber]
                                                [AnalysisParameter.Geomatrices[aSubmatrixNumber]]);
                nextItem(':');
    ...
    if (MimosaSimuSetupDebug) {
        ...
        cout << "    pixel size U " << AnalysisParameter.PixelSizeU[aSubmatrixNumber] << " V "
             << AnalysisParameter.PixelSizeV[aSubmatrixNumber] << endl;
        cout << "    matrix type " << AnalysisParameter.Matrixtype[aSubmatrixNumber] << endl;
        cout << "    # pixels " << AnalysisParameter.PixelsInRaw[aSubmatrixNumber] << " in raw, "
             << AnalysisParameter.PixelsInColumn[aSubmatrixNumber] << " in column" << endl;
        cout << "    min - max # pixels in hit "
             << AnalysisParameter.MinNofPixelsInCluster[aSubmatrixNumber] << " - "
             << AnalysisParameter.MaxNofPixelsInCluster[aSubmatrixNumber] << endl;
        cout << "    range of seed index: " << AnalysisParameter.MinSeedIndex[aSubmatrixNumber]
             << " to " << AnalysisParameter.MaxSeedIndex[aSubmatrixNumber] << endl;
        ...
        for (Int_t il = 0; il < AnalysisParameter.Geomatrices[aSubmatrixNumber]; il++) {
            cout << "    GeoMatrix " << il << " " << AnalysisParameter.Umin[aSubmatrixNumber][il]
                 << " < U < " << AnalysisParameter.Umax[aSubmatrixNumber][il] << "; "
                 << AnalysisParameter.Vmin[aSubmatrixNumber][il] << " < V < "
                 << AnalysisParameter.Vmax[aSubmatrixNumber][il] << endl;
        }
    }
...
void MimosaSimuSetup::ReadConfiguration() {
    ...
    // Modified: VR 2016/06/30, run number is get from DSession and DataSubDirPrefixXXX (XXX = run
    // #) concatenated to DataPath if set
    ...
    // -+-+-+-+-+--+-+-+-+-+--+-+-+-+-+--+-+-+-+-+--+-+-+-+-+--+-+-+-+-+--+-+-+-+-+-
    ReadRunParameters();
    // RunParameter.Number = fSession->GetRunNumber();// VR 20014/06/30 Set the run number from
    // DSession(fRunNumber), which is set by MimosaAnalysis::InitSession(const Int_t TheRun) if (
    // strcmp(RunParameter.DataSubDirPrefix,""))// VR 20014/06/30 if DataSubDirPrefix is given,
    // concatenate it with the run number to the DataPath
    //{
    // sprintf( RunParameter.DataPath, "%s/%s%d/",
    // RunParameter.DataPath,RunParameter.DataSubDirPrefix, RunParameter.Number ); // VR 2014/06/30
    //}
    ...
    // -+-+-+-+-+--+-+-+-+-+--+-+-+-+-+--+-+-+-+-+--+-+-+-+-+--+-+-+-+-+--+-+-+-+-+-
    // ReadExperimentGeometryParameters(); //Valerian 2015/02/02@18h50 : if this method causes bug,
    // tell me so I can correct bug (In a previous commit it was comment ?!)
    ...
    if (!strcmp(fFieldName, "Inputs"))
        ReadPlaneParameters(fAddedPlanes); // contains iterative call to subsequent planes
    ...
    if (!strcmp(fFieldName, "LadderID"))
        ReadLadderParameters(fAddedLadders); // contains iterative call to subsequent ladders
    ...
    if (!strcmp(fFieldName, "PixelSizeU")) {
        ...
        printf(
            "WARNING in ReadAnalysisParameter: Missing submatrix parameters (PixelSize, ...)!\n");
    }
...
//==================================================================
MimosaSimuSetup::MimosaSimuSetup(const MimosaSimuSetup &c) { copy(c); }
//==================================================================
...
void MimosaSimuSetup::CheckExpSetupParameters(void) {
    ...
    if (TrackerParameter.ExpSetup == TString("") ||
        ...
        cout << endl;
        cout << "The experimental setuo parameter ExpSetup needs to be specified to one of the "
                "following values."
             << endl;
        cout << " - Beam-Test," << endl;
        for (int isource = 0; isource < int(_Source_list.size()); isource++)
            cout << " - " << _Source_list[isource].Data() << "," << endl;
...
void MimosaSimuSetup::CheckBeamTestParameters(void) {
    ...
    if (TrackerParameter.ExpSetup == TString("")) {
        TrackerParameter.ExpSetup = TString("Beam-Test");
        cout << "ExpSetup  parameter not specified. Setting it to default value:  "
             << TrackerParameter.ExpSetup.Data() << endl;
    } else if (TrackerParameter.ExpSetup == TString("Beam-Test") ||
               TrackerParameter.ExpSetup == TString("beam-test")) {
        TrackerParameter.ExpSetup = TString("Beam-Test");
        cout << "ExpSetup  parameter set to user value:  " << TrackerParameter.ExpSetup.Data()
             << endl;
    }

    if (TrackerParameter.BeamType == TString("")) {
        TrackerParameter.BeamType = TString("pion-");
        cout << "    BeamType             parameter not specified. Setting it to defaul value:  "
             << TrackerParameter.BeamType.Data() << endl;
    } else
        cout << "    BeamType             parameter set to user value: "
             << TrackerParameter.BeamType.Data() << endl;

    if (TrackerParameter.BeamNparticles == -1.0) {
        TrackerParameter.BeamNparticles = 1.0;
        cout << "    BeamNparticles       parameter not specified. Setting it to defaul value:  "
             << TrackerParameter.BeamNparticles << endl;
    } else if (TrackerParameter.BeamNparticles < 0.0) {
        TrackerParameter.BeamNparticles = 1.0;
        cout << "    BeamNparticles       parameter set to negative value. Setting it to defaul "
                "value:  "
             << TrackerParameter.BeamNparticles << endl;
    } else
        cout << "    BeamNparticles       parameter set to user value: "
             << TrackerParameter.BeamNparticles << endl;

    if (TrackerParameter.BeamRandNparticles) {
        cout << "        Generate number of particle with Poisson distribution with mean value :  "
             << TrackerParameter.BeamNparticles << endl;
    } else
        cout << "        Generate exactly " << TrackerParameter.BeamNparticles
             << " particles per event" << endl;

    if (TrackerParameter.BeamMomentum == -1.0) {
        TrackerParameter.BeamMomentum = 120.0;
        cout << "    BeamMomentum         parameter not specified. Setting it to defaul value: "
             << TrackerParameter.BeamMomentum << " GeV/c" << endl;
    } else if (TrackerParameter.BeamMomentum < 0) {
        TrackerParameter.BeamMomentum = 120.0;
        cout << "    BeamMomentum         parameter set to user value is negative. Setting it to "
                "defaul value: "
             << TrackerParameter.BeamMomentum << " GeV/c" << endl;
    } else
        cout << "    BeamMomentum         parameter set to user value: "
             << TrackerParameter.BeamMomentum << " GeV/c" << endl;

    if (TrackerParameter.BeamDirection == G4ThreeVector(0.0, 0.0, 1.0)) {
        cout << "    BeamDirection        parameter not specified. Setting it to defaul value: ("
             << TrackerParameter.BeamDirection(0) << "," << TrackerParameter.BeamDirection(1) << ","
             << TrackerParameter.BeamDirection(2) << ")" << endl;
    } else {
        // Normalizing Beam direction vector to unit module
        if (TMath::Abs(TrackerParameter.BeamDirection.mag()) < 1.0e-8) {
            G4cerr << "BeamDirection    vector has zero module. Check your inputs. Exiting now!!!"
                   << G4endl;
            assert(false);
        }
        TrackerParameter.BeamDirection /= TrackerParameter.BeamDirection.mag();
        cout << "    BeamDirection        parameter set to user value: ("
             << TrackerParameter.BeamDirection(0) << "," << TrackerParameter.BeamDirection(1) << ","
             << TrackerParameter.BeamDirection(2) << ")" << endl;
    }

    if (TrackerParameter.BeamAngularSpread == G4ThreeVector(0.0, 0.0, 1.0)) {
        cout << "    BeamAngularSpread    parameter not specified. Setting it to defaul value: ("
             << TrackerParameter.BeamAngularSpread(0) << ","
             << TrackerParameter.BeamAngularSpread(1) << ")" << endl;
    } else if (TrackerParameter.BeamAngularSpread(0) < 0.0 ||
               TrackerParameter.BeamAngularSpread(1) < 0.0) {
        TrackerParameter.BeamAngularSpread.setX(0.0);
        TrackerParameter.BeamAngularSpread.setY(0.0);
        cout << "    BeamAngularSpread    parameter has negative value. Setting it to defaul "
                "value: ("
             << TrackerParameter.BeamAngularSpread(0) << ","
             << TrackerParameter.BeamAngularSpread(1) << ")" << endl;
    } else
        cout << "    BeamAngularSpread    parameter set to user value: ("
             << TrackerParameter.BeamAngularSpread(0) << ","
             << TrackerParameter.BeamAngularSpread(1) << ")" << endl;

    if (TrackerParameter.BeamMomentumSpread == -1) {
        TrackerParameter.BeamMomentumSpread = 1.0e-10;
        cout << "    BeamMomentumSpread   parameter not specified. Setting it to defaul value: "
             << TrackerParameter.BeamMomentumSpread << endl;
    } else if (TrackerParameter.BeamMomentumSpread < 0.0) {
        TrackerParameter.BeamMomentumSpread = 0.0;
        cout
            << "    BeamMomentumSpread   parameter has negative value. Setting it to defaul value: "
            << TrackerParameter.BeamMomentumSpread << endl;
    } else
        cout << "    BeamMomentumSpread   parameter set to user value: "
             << TrackerParameter.BeamMomentumSpread << endl;

    if (TrackerParameter.BeamOrigin == G4ThreeVector(0.0, 0.0, -1.0e+6) ||
        TrackerParameter.BeamOriginSpread(0) < 0.0 || TrackerParameter.BeamOriginSpread(1) < 0.0) {
        cout << "    BeamOrigin or BeamOriginSpread  parameters not specified. Will perform "
                "uniform generation in lowest Z face of the world volume cube"
             << endl;
    } else {
        cout << "    BeamOrigin and BeamOriginSpread  parameters specified by user. Will perform "
                "Gaussian generation at BeamOrigin position with BeamOriginSpread."
             << endl;
        cout << "    BeamOrigin           parameter set to user value: ("
             << TrackerParameter.BeamOrigin(0) / 1000 << ","
             << TrackerParameter.BeamOrigin(1) / 1000 << ","
             << TrackerParameter.BeamOrigin(2) / 1000 << ") mm" << endl;
        cout << "    BeamOriginSpread     parameter set to user value: ("
             << TrackerParameter.BeamOriginSpread(0) / 1000 << ","
             << TrackerParameter.BeamOriginSpread(1) / 1000 << ") mm" << endl;
    }
...
void MimosaSimuSetup::CheckSourceParameters(void) {
    ...
    if (TrackerParameter.SourcePosition == G4ThreeVector(-999.0, -999.0, -999.0)) {
        ...
        cout << "    SourcePosition         parameter set to user value: ("
             << TrackerParameter.SourcePosition(0) << "," << TrackerParameter.SourcePosition(1)
             << "," << TrackerParameter.SourcePosition(2) << ") mm" << endl;

    if (TrackerParameter.SourceTilt == G4ThreeVector(-999.0, -999.0, -999.0)) {
        TrackerParameter.SourceTilt == G4ThreeVector(0.0, 0.0, 0.0);
        cout << "    SourceTilt             parameter not specified. Setting it to default value: ("
             << TrackerParameter.SourceTilt(0) << "," << TrackerParameter.SourceTilt(1) << ","
             << TrackerParameter.SourceTilt(2) << ") deg" << endl;
    } else
        cout << "    SourceTilt             parameter set to user value: ("
             << TrackerParameter.SourceTilt(0) << "," << TrackerParameter.SourceTilt(1) << ","
             << TrackerParameter.SourceTilt(2) << ") deg" << endl;
    ...
    if (TrackerParameter.SourceRadius == -1.0) {
        ...
        cout << "    SourceRadius           parameter set to user value: "
             << TrackerParameter.SourceRadius << " mm" << endl;

    if (TrackerParameter.SourceActivity == -1.0) {
        cout << "    SourceActivity         parameter not specified. Exiting now!!!" << endl;
        assert(false);
    } else
        cout << "    SourceActivity         parameter set to user value: "
             << TrackerParameter.SourceActivity << " decays/sec" << endl;

    if (TrackerParameter.SourceSensorROTime == -1.0) {
        cout << "    SourceSensorROTime     parameter not specified. Exiting now!!!" << endl;
        assert(false);
    } else
        cout << "    SourceSensorROTime     parameter set to user value: "
             << TrackerParameter.SourceSensorROTime << " usec" << endl;
...
void MimosaSimuSetup::CheckPlaneDigitizationParameters(Int_t aPlaneNumber) {
    ...
    if (pPlaneParameter[aPlaneNumber].PlaneDigitization == TString("")) {
        cout << " WARNING: NO DIGITIZATION WILL BE PERFORMED for plane " << aPlaneNumber + 1
             << endl;
        return;
    }
    ...
    if (pPlaneParameter[aPlaneNumber].AnalysisMode < 2) {
        // Strip readout
        cout << "  WARNING: Not able to perform digitization for analysis mode < 2. No "
                "digitization will be performed!!!"
             << endl;
        ...
        cout << "Performing digitization with analogue output for plane " << aPlaneNumber + 1
             << ". Using model " << pPlaneParameter[aPlaneNumber].PlaneDigitization << endl;
        ...
        if ((pPlaneParameter[aPlaneNumber].PlaneDigitizeNoise > 0.0) &&
            ...
            (pPlaneParameter[aPlaneNumber].PlaneDigitizeADCMin <
             pPlaneParameter[aPlaneNumber].PlaneDigitizeADCMax)) {
            cout << "  Using the following digitization parameters:" << endl;
            cout << "    - Sensor noise (electrons)        = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeNoise << endl;
            cout << "    - Sensor calibration (elec/Volts) = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeCalib << endl;
            cout << "    - Sensor number of ADC bits       = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeADCbits << endl;
            cout << "    - Sensor ADC dynamic range        = ("
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeADCMin << ","
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeADCMax << ") volts" << endl;
        } else {
            cout << "  WARNING: The below parameters either not set of set to wrong alues:" << endl;
            cout << "    - Sensor noise (PlaneDigitizeNoise)                                   = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeNoise << " (electrons)" << endl;
            cout << "    - Sensor calibration (PlaneDigitizeCalib)                             = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeCalib << " (elec/Volt)" << endl;
            cout << "    - Sensor number of ADC bits (PlaneDigitizeADCbits)                    = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeADCbits << endl;
            cout << "    - Sensor ADC dynamic range  (PlaneDigitizeADCMin,PlaneDigitizeADCMax) = ("
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeADCMin << ","
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeADCMax << ") volts" << endl;
        ...
        cout << "Performing digitization with digital output for plane " << aPlaneNumber + 1
             << ". Using model " << pPlaneParameter[aPlaneNumber].PlaneDigitization << endl;
        ...
        if (pPlaneParameter[aPlaneNumber].PlaneDigitizeNoise > 0.0 &&
            ...
            cout << "    - Sensor noise (electrons)       = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeNoise << endl;
            cout << "    - Threshold (x noise)            = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeThre << endl;
            cout << "    - Sensor average Noise occupancy = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeOcc << endl;
        } else {
            cout << "  WARNING: The below parameters either not set of set to wrong alues:" << endl;
            cout << "    - Sensor noise (electrons)       = "
                 << pPlaneParameter[aPlaneNumber].PlaneDigitizeNoise << endl;
