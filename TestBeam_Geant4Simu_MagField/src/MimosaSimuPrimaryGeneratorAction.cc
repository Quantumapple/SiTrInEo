...
MimosaSimuPrimaryGeneratorAction::MimosaSimuPrimaryGeneratorAction(MimosaSimuSetup *TheSetup)
    : G4VUserPrimaryGeneratorAction(), fSetup(TheSetup) {
...
MimosaSimuPrimaryGeneratorAction::~MimosaSimuPrimaryGeneratorAction() {
    ...
    // cout << "MimosaSimuPrimaryGeneratorAction::~MimosaSimuPrimaryGeneratorAction:: All OK!!!" <<
    // endl;
}
...
void MimosaSimuPrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent) {
    ...
    if (fSetup->TrackerParameter.ExpSetup == TString("Beam-Test"))
        GenerateBeamTestPrimaries(anEvent);
    else if (fSetup->CheckIfSourceExists(fSetup->TrackerParameter.ExpSetup))
        GenerateSourcePrimaries(anEvent);
...
void MimosaSimuPrimaryGeneratorAction::GenerateBeamTestPrimaries(G4Event *anEvent) {
    ...
    if (fSetup->TrackerParameter.BeamRandNparticles) {
        ...
        TheNparticles = nParticles;

    if (Myverbose) {
        G4cout << G4endl;
        G4cout << "Generating " << TheNparticles << " particles for event "
               << anEvent->GetEventID() + 1 << " (" << nParticles << ")" << G4endl;
    }
    ...
    for (G4int ipar = 0; ipar < TheNparticles; ipar++) {
        ...
        mySeed++;
        if (mySeed == myOldSeed)
            std::cout << "CAUTION SAME SEED AS BEFORE !!!!!!!!!!!!!!!!!!!" << std::endl;
        myOldSeed = mySeed;
        ...
        if (Myverbose) {
            ...
            G4cout << "   Position  = (" << GenerationPostion(0) / mm << ","
                   << GenerationPostion(1) / mm << "," << GenerationPostion(2) / mm << ") mm"
                   << G4endl;
            G4cout << "   Direction = (" << MomentumDirection(0) / mm << ","
                   << MomentumDirection(1) / mm << "," << MomentumDirection(2) / mm << ") mm"
                   << G4endl;
...
void MimosaSimuPrimaryGeneratorAction::GenerateSourcePrimaries(G4Event *anEvent) {
    ...
    if (Myverbose) {
        G4cout << G4endl;
        G4cout << "Generating " << TheNparticles << " particles for event "
               << anEvent->GetEventID() + 1 << " (" << nParticles * 0.5 << ")" << G4endl;
    }
    ...
    for (G4int ipar = 0; ipar < TheNparticles; ipar++) {
        ...
        GenerationPostion = G4ThreeVector(radius * cos(phi_pos), radius * sin(phi_pos),
...
void MimosaSimuPrimaryGeneratorAction::InitGenerationParameters(void) {
    ...
    if (fSetup->TrackerParameter.ExpSetup == TString("Beam-Test")) {
        ...
        if (fSetup->TrackerParameter.BeamOrigin == G4ThreeVector(0.0, 0.0, -1.0e+6) ||
            ...
            UniformOriginGeneration = true;

        if (UniformOriginGeneration)
            DoUniformBeamTestInit();
        else
            DoBeamSpotBeamTestInit();

        G4cout << "Mean energy of " << TheBeamMomentum / GeV << " GeV, with smearing of "
               << TheBeamMomentumSpread * 100.0 << " %" << G4endl;
        G4cout << "Mean angular dispersion RMS in (X',Y') = (" << TheBeamAngularSpreadX << ","
               << TheBeamAngularSpreadY << ")" << G4endl;
        G4cout << G4endl;
    } else if (fSetup->CheckIfSourceExists(fSetup->TrackerParameter.ExpSetup)) {
...
void MimosaSimuPrimaryGeneratorAction::DoUniformBeamTestInit(void) {
    ...
    G4cerr << "BeamOrigin or BeamOriginSpread  parameters not specified. Will perform uniform "
              "generation in lowest Z face of the world volume cube"
           << G4endl;
    ...
    if (TheBeamDirection.dot(G4ThreeVector(0.0, 0.0, 1.0)) < 0.0)
        TheBeamOrigin(2) = +worldZHalfLength - 0.001 * worldZHalfLength;
    ...
    G4cout << "X and Y origin with flat distribution in XRange x YRange of ("
           << -TheBeamOriginSpreadX / mm << "," << TheBeamOriginSpreadX / mm << ") x ("
           << -TheBeamOriginSpreadX / mm << "," << TheBeamOriginSpreadX / mm << ") mm^2" << G4endl;
    G4cout << G4endl;
...
void MimosaSimuPrimaryGeneratorAction::DoBeamSpotBeamTestInit(void) {
    ...
    G4cout << G4endl;
    G4cout << "The beam direction = (" << TheBeamDirection(0) << "," << TheBeamDirection(1) << ","
           << TheBeamDirection(2) << ")" << G4endl;
    G4cout << "X' director vector = (" << PerpXprime(0) << "," << PerpXprime(1) << ","
           << PerpXprime(2) << ")" << G4endl;
    G4cout << "Y' director vector = (" << PerpYprime(0) << "," << PerpYprime(1) << ","
           << PerpYprime(2) << ")" << G4endl;
    ...
    G4cout << G4endl;
    G4cout << "Beam particles will be generated at origin = ( " << TheBeamOrigin(0) / mm << ","
           << TheBeamOrigin(1) / mm << "," << TheBeamOrigin(2) / mm << ") mm" << G4endl;
    G4cout << " with (X',Y') spread of ( " << TheBeamOriginSpreadX / mm << ","
           << TheBeamOriginSpreadY / mm << ") mm" << endl;
    G4cout << G4endl;
...
void MimosaSimuPrimaryGeneratorAction::DoSourceInit(void) {
    ...
    if (IsBetaSource(SourceType))
        InitBetaSourceSpectrum(SourceType);
    else if (IsXRaySource(SourceType))
        InitXRaySourceSpectrum(SourceType);
...
void MimosaSimuPrimaryGeneratorAction::CheckIfParticleIsInMap(TString AParticle) {
    ...
    if (!IsInMap) {
        cout << endl;
        cout << "The specified particle type " << TheParticle.Data()
             << " is not in internal database:" << endl;
...
void MimosaSimuPrimaryGeneratorAction::InitBetaSourceSpectrum(TString SourceName) {
    ...
    if (SourceName == TString("Source-Sr90")) {
        ...
        double p_Sr_90_limit =
            sqrt(pow(Q_Sr_90, 2) - pow(particle_map[TheParticle]->GetPDGMass(), 2));
        double Q_Y_90 = 2.280 * MeV;
        double p_Y_90_limit =
            sqrt(pow(Q_Y_90, 2) - pow(particle_map[TheParticle]->GetPDGMass(), 2));
        ...
        // cout << "R_mom_Sr_90  (" << R_mom_Sr_90[0]/MeV  << "," << R_mom_Sr_90[1]/MeV  << ") MeV"
        // << endl; cout << "R_mom_Y_90   (" << R_mom_Y_90[0]/MeV   << "," << R_mom_Y_90[1]/MeV   <<
        // ") MeV" << endl; cout << "R_mom_SrY_90 (" << R_mom_SrY_90[0]/MeV << "," <<
        // R_mom_SrY_90[1]/MeV << ") MeV" << endl;

        TH1F h_MomSpec_Sr_90("h_MomSpec_Sr_90",
                             "#beta^{-} momentum spectrum from ^{90}_{38}Sr #rightarrow "
                             "^{90}_{39}Y + e^{-} + #bar{#nu}_{e}",
                             Nbins, R_mom_SrY_90[0], R_mom_SrY_90[1]);
        TH1F h_MomSpec_Y_90("h_MomSpec_Y_90",
                            "#beta^{-} momentum spectrum from ^{90}_{39}Sr #rightarrow ^{90}_{39}Y "
                            "#rightarrow ^{90}_{40}Zr + e^{-} + #bar{#nu}_{e}",
                            Nbins, R_mom_SrY_90[0], R_mom_SrY_90[1]);

        h_SourceBetaSpectrum =
            TH1F("h_SourceBetaSpectrum",
                 "p spectrum in T_{0} from ^{90}_{38}Sr and ^{90}_{39}Y #beta decays", Nbins,
                 R_mom_SrY_90[0], R_mom_SrY_90[1]);
...
G4double MimosaSimuPrimaryGeneratorAction::Mom_beta_spectrum_pdf(G4double p, G4double Q, G4double Z,
                                                                 bool InclFermi) {
...
G4double MimosaSimuPrimaryGeneratorAction::GetSourceRandomMomentum(void) {
    ...
    if (IsBetaSource(SourceType))
        Momentum = GetBetaSourceRandomMomentum();
    else if (IsXRaySource(SourceType))
        Momentum = GetXRaySourceRandomMomentum();
    else
        Momentum = 0.0;
...
G4double MimosaSimuPrimaryGeneratorAction::GetXRaySourceRandomMomentum() {
    ...
    if (XRayLinesCumulProb.size() == 1)
        Momentum = XRayLinesEnergy[0];
    else {
