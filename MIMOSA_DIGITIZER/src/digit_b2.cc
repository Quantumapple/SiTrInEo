...
// ClassImp(DIGIT_B2)
DIGIT_B2::DIGIT_B2()
    : TNamed("DIGIT_B2", "DIGIT_B2 title"), GlobalSeed(1), Mapping(1), AnalysisMode(3),
      PitchU(18.4), PitchV(18.4), shiftU(0.0), shiftV(0.0), NpixelsU(576), NpixelsV(1152),
      EffectiveEpi(10.0), NoiseElectrons(14.0), Calib(1.0), ADCbits(1), ADCRangeMin(0.0),
      ADCRangeMax(1.0), TheTransportModel("HR2016") {
    //
    // default constructor
    Initialize();
...
DIGIT_B2::DIGIT_B2(char *name, char *title)
    : TNamed(name, title), GlobalSeed(1), Mapping(1), AnalysisMode(3), PitchU(18.4), PitchV(18.4),
      shiftU(0.0), shiftV(0.0), NpixelsU(576), NpixelsV(1152), EffectiveEpi(10.0),
      NoiseElectrons(14.0), Calib(1.0), ADCbits(1), ADCRangeMin(0.0), ADCRangeMax(1.0),
...
DIGIT_B2::DIGIT_B2(char *name, char *title, Int_t myglobalseed, TString TransportModel)
    : TNamed(name, title), GlobalSeed(myglobalseed), Mapping(1), AnalysisMode(3), PitchU(18.4),
      PitchV(18.4), shiftU(0.0), shiftV(0.0), NpixelsU(576), NpixelsV(1152), EffectiveEpi(10.0),
      NoiseElectrons(14.0), Calib(1.0), ADCbits(1), ADCRangeMin(0.0), ADCRangeMax(1.0),
...
//______________________________________________________________________________
//
DIGIT_B2::DIGIT_B2(char *name, char *title, Int_t myglobalseed, Int_t aMapping, Int_t aAnalysisMode,
                   Double_t aPitchU, Double_t aPitchV, Double_t ashiftU, Double_t ashiftV,
                   Int_t aNpixelsU, Int_t aNpixelsV, Double_t aEffectiveEpi,
                   Double_t aNoiseElectrons, TString TransportModel, Double_t aCalib,
                   Int_t aADCbits, Double_t aADCRangeMin, Double_t aADCRangeMax)
    : TNamed(name, title), GlobalSeed(myglobalseed), Mapping(aMapping), AnalysisMode(aAnalysisMode),
      PitchU(aPitchU), PitchV(aPitchV), shiftU(ashiftU), shiftV(ashiftV), NpixelsU(aNpixelsU),
      NpixelsV(aNpixelsV), EffectiveEpi(aEffectiveEpi), NoiseElectrons(aNoiseElectrons),
      Calib(aCalib), ADCbits(aADCbits), ADCRangeMin(aADCRangeMin), ADCRangeMax(aADCRangeMax),
      TheTransportModel(TransportModel) {
...
//______________________________________________________________________________
//
void DIGIT_B2::SetSensorParameters(Double_t aPitchU, Double_t aPitchV, Double_t ashiftU,
                                   Double_t ashiftV, Int_t aNpixelsU, Double_t aNpixelsV,
                                   Double_t aEffectiveEpi, Double_t aNoiseElectrons, Int_t aMapping,
                                   Int_t aAnalysisMode, TString aTransportModel, Double_t aCalib,
                                   Int_t aADCbits, Double_t aADCRangeMin, Double_t aADCRangeMax) {
...
Double_t Lorentz2D(Double_t *x, Double_t *par) {
    ...
    if (par[0] > 0) {
        ...
        return par[3] * par[0] / Pi /
               ((x[0] - par[1]) * (x[0] - par[1]) + (x[1] - par[2]) * (x[1] - par[2]) +
                par[0] * par[0]);
    } else {
        return 0;
    }
...
Double_t SumGausLorentz2D(Double_t *x, Double_t *par) {
    ...
    if ((par[2] != 0.0) && (par[4] != 0.0)) {
        ...
        return par[0] * (TMath::Exp(-(rx * rx + ry * ry) / 2.0) +
                         par[8] * par[5] / Pi /
                             ((x[0] - par[6]) * (x[0] - par[6]) +
                              (x[1] - par[7]) * (x[1] - par[7]) + par[5] * par[5]));
    } else {
        return 0;
    }
}
...
void DIGIT_B2::ComputePixelPositionUV_FromColRow(Int_t col, Int_t lin, Double_t &u, Double_t &v) {
    ...
    switch (Mapping) {
        ...
        case 2: {
            ...
            u = ((2 * col - NpixelsU + 1) * PitchU) / 2;
            if (col % 2 == 0)
                v = (((lin - NpixelsV / 2.0 + 1. / 2.) * PitchV)) + 0.30 * PitchV; // clm 2012.11.24
            else
                v = (((lin - NpixelsV / 2.0 + 1. / 2.) * PitchV)) - 0.19 * PitchV; // clm 2012.11.24
            break;
        }
        case 3: {
            ...
            if (lin == 7 && col == 10)
                ...
                u = -9999; //((2*col+2 - NpixelsU + 1 ) * PitchU)/2 ;
            v = ((2 * lin - NpixelsV + 1) * PitchV) / 2;
            break;
        }
        case 4: {
        ...
        case 5: {
            ...
            if (col % 2 == 0)
                v = 0.5 * (NpixelsV - 2 * lin - 2 * (fraction)) * PitchV;
            else
                v = 0.5 * (NpixelsV - 2 * lin - 2 * (1 - fraction)) * PitchV;
            break;
        }
        case 6: {
            // Mapping FSBB-bis
        ...
        case 7: {
            ...
            u = ((2 * col - NpixelsU + 1) * PitchU) / 2;
            if (col % 2 == 0)
                v = (((lin - NpixelsV / 2.0 + 1. / 2.) * PitchV)) - 0.50 * PitchV; // correct +0.5
            else
                v = (((lin - NpixelsV / 2.0 + 1. / 2.) * PitchV)) - 0.00 * PitchV; // correct -0.5
            break;
        }
    }
...
void DIGIT_B2::ComputePixelPositionColRow_FromUV(Double_t u, Double_t v, double &col, double &lin) {
    ...
    // Compute the 2D position of a strip in the variables u and v to the set of variables column
    // and line The strip position depends on the mapping, look below.
    //
    ...
    switch (Mapping) {
        ...
        case 2: {
            ...
            col = (u / PitchU) + ((NpixelsU - 1) / 2.);
            if (int(col) % 2 == 0)
                lin = (v / PitchV) + ((NpixelsV - 1) / 2.) - 0.30;
            else
                lin = (v / PitchV) + ((NpixelsV - 1) / 2.) + 0.19;
            break;
        }
        case 3: {
            // Clm Mapping For M32 L8_2
        ...
        case 5: {
            ...
            if (IntCol % 2 == 0)
                lin = ((NpixelsV - 2 * (fraction)) / 2.0) - (v / PitchV);
            else
                lin = ((NpixelsV - 2 * (1 - fraction)) / 2.0) - (v / PitchV);
        ...
        case 7: {
            ...
            col = (u / PitchU) + ((NpixelsU - 1) / 2.);
            if (int(col) % 2 == 0)
                lin = (v / PitchV) + ((NpixelsV - 1) / 2.) + 0.50; // correct -0.5
            else
                lin = (v / PitchV) + ((NpixelsV - 1) / 2.) + 0.0; // correct +0.5
            break;
        }
    }
...
Int_t DIGIT_B2::GetPixelNumber(Double_t Xpos, Double_t Ypos) {
  ...
  if(ConventionOld) {
    ...
    if ((Xpos < RU[0] - epsilon) || (Xpos > RU[1] + epsilon) || (Ypos < RV[0] - epsilon) ||
        (Ypos > RV[1] + epsilon)) {
...
void DIGIT_B2::GetXYPixelNumber(Int_t &Xpix, Int_t &Ypix, Int_t PixelNumber) {
...
//______________________________________________________________________________
//
// get the X and Y position (float) of the pixel from the pixel number (int), taking into account
// diode shift.
void DIGIT_B2::GetXYPixelCenter(Double_t &Xpix, Double_t &Ypix, Int_t PixelNumber) {
...
//______________________________________________________________________________
//
int DIGIT_B2::GetGlobalIdxFromColRow(int col, int row) { return col + NpixelsU * row; }
//______________________________________________________________________________
//
void DIGIT_B2::DefineTransportModel(void) {
    ...
    if ((TheTransportModel == TString("AMSlorgaus")) || (TheTransportModel == TString("3"))) {
        ...
        // par[7] = y0
        // par[8] = norm
        Double_t Norm1Value = LorGaussModel_Norm1_Cp0 + PitchU * LorGaussModel_Norm1_Cp1 +
                              PitchU * PitchU * LorGaussModel_Norm1_Cp2;
        ...
        if (!mymodel2D) {
            mymodel2D = new TF2("funlor2d", SumGausLorentz2D, -rangelimit_inpitchunit * PitchU,
                                rangelimit_inpitchunit * PitchU, -rangelimit_inpitchunit * PitchV,
                                rangelimit_inpitchunit * PitchV, 9);
            mymodel2D->SetParNames("Norm_1", "x0_1", "sigma_x_1", "y0_1", "sigma_y_1", "Gamma",
                                   "x0", "y0", "norm");
        } else {
        ...
        Double_t params1[] = {Norm1Value, 0.0, sigmaValue, 0.0,      sigmaValue,
                              Cvalue,     0.0, 0.0,        NormValue};
    ...
    //-----------------------------------------------------
        ...
        if (!mymodel1D_1st) {
            ...
            mymodel1D_1st->SetParNames("Norm_g", "x0_g", "sigma_g", "Gamma_lor", "x0_lor",
                                       "norm_lor");
        } else {
        ...
        if (!mymodel1D_2nd) {
            ...
            mymodel1D_2nd->SetParNames("Norm_g", "x0_g", "sigma_g", "Gamma_lor", "x0_lor",
                                       "norm_lor");
        } else {
            mymodel1D_2nd->SetRange(0, 4.0 * PitchU);
        }

        Double_t params1[] = {Norm_g_1st,    x0_g_1st,   sigma_g_1st,
                              Gamma_lor_1st, x0_lor_1st, norm_lor_1st};
        Double_t params2[] = {Norm_g_2nd,    x0_g_2nd,   sigma_g_2nd,
                              Gamma_lor_2nd, x0_lor_2nd, norm_lor_2nd};
    ...
    //-----------------------------------------------------
        ...
        cout << endl;
        cout << "Digitization Model " << TheTransportModel.Data()
             << "  doesn't correspond to list:" << endl;
...
//______________________________________________________________________________
//
void DIGIT_B2::Compute_CollectedCharge(Double_t SegmentSize, Double_t InputUpos, Double_t InputVpos,
                                       Double_t OutputUpos, Double_t OutputVpos,
                                       Double_t Edeposited, Double_t RandTheta, Double_t RandPhi,
    ...
    Double_t totallentgh = sqrt(pow(InputUpos - OutputUpos, 2) + pow(InputVpos - OutputVpos, 2) +
                                pow(EffectiveEpi, 2));
    ...
    for (Int_t i = 0; i < fNSegment; i++) {
        ...
        for (Int_t j = 0; j < Npix; j++) {
            ...
            if ((TheTransportModel == TString("1")) ||
                (TheTransportModel == TString("AMSlorgaus")) ||
                (TheTransportModel == TString("3"))) {
                Pixelproba[j] = mymodel2D->Eval(xeval, yeval);
            } else {
                if ((fabs(xeval) <= PitchU) && (fabs(yeval) <= PitchV))
            ...
            if ((colj >= 0 && colj <= NpixelsU - 1) && (rowj >= 0 && rowj <= NpixelsV - 1))
                PixelNumber[j] = GetGlobalIdxFromColRow(colj, rowj);
            else
                PixelNumber[j] = -1;

            // if( (PixelposX[j] < RU[0]) || (PixelposX[j] > RU[1]) || (PixelposY[j] < RV[0]) ||
            // (PixelposY[j] > RV[1]) ) {
        ...
        for (Int_t j = 0; j < Npix; j++) {
            ...
            if (PixelNumber[j] >= 0) {
                ...
                Bool_t found = false;
                Int_t k      = 0; // j = indice de la matrice 5x5  / k = indice sur fPixelMap / i =
                                  // indice sur les segments
...
//______________________________________________________________________________
//
void DIGIT_B2::Compute_digitisation(vector<Int_t> fPixelMap, vector<Float_t> fAnalogChargeMap,
                                    vector<Int_t> &fDigitalChargeMap,
...
//______________________________________________________________________________
//
void DIGIT_B2::Transport_Noise_Digitisation(
    Double_t SegmentSize, Double_t InputUpos, Double_t InputVpos, Double_t OutputUpos,
    Double_t OutputVpos, Double_t Edeposited, Double_t RandTheta, Double_t RandPhi,
    vector<Int_t> &fPixelMap, vector<Float_t> &fAnalogChargeMap, vector<Int_t> &fDigitalChargeMap,
    Double_t Discri_Threshold_electrons)

{

    //----------compute collected charge:
    Compute_CollectedCharge(SegmentSize, InputUpos, InputVpos, OutputUpos, OutputVpos, Edeposited,
                            RandTheta, RandPhi, fPixelMap, fAnalogChargeMap, fDigitalChargeMap);
    ...
    Compute_digitisation(fPixelMap, fAnalogChargeMap, fDigitalChargeMap,
...
void DIGIT_B2::CheckADCRanges(void) {
    ...
    if (AnalysisMode == 2) {
        if (ADCRangeMin >= ADCRangeMax) {
            cout << endl;
            cout << " ERROR: ADCRangesMax <= ADCRangesMin. Check your inputs. Exiting now!!!"
                 << endl;
            cout << endl;
...
void DIGIT_B2::Run(Long64_t aGlobalSeed) {
    ...
    // Theta is the polar angle (theta = 0 -> orthogonal track ; tetha ~ pi/2 -> low incident angle
    // track phi is the azimuthal angle (phi = 0 ->  track in the U direction). all dimensions are
    // in microns
    ...
    SetSensorParameters(aPitchU, aPitchV, ashiftU, ashiftV, aNpixelsU, aNpixelsV, aEffectiveEpi,
                        aNoiseElectrons, aMapping, aAnalysisMode, aTransportModel);
    ...
    for (Int_t Nev = 0; Nev < Nevents; Nev++) {
        ...
        Transport_Noise_Digitisation(SegmentSize, InputUpos, InputVpos, OutputUpos, OutputVpos,
                                     -1.0, RandTheta, RandPhi, fPixelMap, fAnalogChargeMap,
                                     fDigitalChargeMap, Discri_Threshold_electrons);
...
//______________________________________________________________________________
//
void DIGIT_B2::Run2(Long64_t Nevents, Int_t aMapping, Int_t aAnalysisMode, Int_t aNpixelsU,
                    Int_t aNpixelsV, Double_t aPitchU, Double_t aPitchV, Double_t aEffectiveEpi,
                    Double_t aNoiseElectrons, const char *aTransportModel, Double_t Trk_thetaLoc,
                    Long64_t aGlobalSeed, const char *output_file, bool verbose) {
    ...
    SetSensorParameters(aPitchU, aPitchV, ashiftU, ashiftV, aNpixelsU, aNpixelsV, aEffectiveEpi,
                        aNoiseElectrons, aMapping, aAnalysisMode, TString(aTransportModel));

    // Double_t  Discri_Threshold_electrons = Thre_Noise*NoiseElectrons;

    Double_t SegmentSize =
        GetEffectiveEpi() / 20.0; // size of segment for charge deposition in the epi layer
    ...
    if (verbose) {
        ...
        // cout << " - Threshold       = " << Thre_Noise << "/" << Discri_Threshold_electrons << "
        // xNoise/electrons" << endl;
    ...
    TH1F *h_effic_vs_Thr =
        new TH1F(HistName.Data(), HistTitle.Data(), N_Thr_steps, R_Thr[0], R_Thr[1]);
    ...
    TH1F *h_AnalogResolutionU_vs_Thr =
        new TH1F(HistName.Data(), HistTitle.Data(), N_Thr_steps, R_Thr[0], R_Thr[1]);
    ...
    HistTitle = TString("Analog resolution vs threshold");
    TH1F *h_AnalogResolutionV_vs_Thr =
        new TH1F(HistName.Data(), HistTitle.Data(), N_Thr_steps, R_Thr[0], R_Thr[1]);
    ...
    TH1F *h_DigitalResolutionU_vs_Thr =
        new TH1F(HistName.Data(), HistTitle.Data(), N_Thr_steps, R_Thr[0], R_Thr[1]);
    ...
    HistTitle = TString("Digital resolution vs threshold");
    TH1F *h_DigitalResolutionV_vs_Thr =
        new TH1F(HistName.Data(), HistTitle.Data(), N_Thr_steps, R_Thr[0], R_Thr[1]);
    ...
    for (int ithr = 0; ithr < N_Thr_steps; ithr++) {
        double Threshold_Times_Noise =
            R_Thr[0] + (ithr + 0.5) * (R_Thr[1] - R_Thr[0]) / N_Thr_steps;
        sprintf(ytitle, "%.1f", Threshold_Times_Noise);

        HistName = TString("hmultiplicity_Thr") + long(ithr + 1);
        HistTitle =
            TString("cluster multiplicity for Thr = ") + TString(ytitle) + TString(" #times Noise");
        hmultiplicity[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_Mult, R_Mult[0], R_Mult[1]);
        ...
        HistTitle =
            TString("Seed pixel charge for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_SeedCharge[ithr] = new TH1F(HistName.Data(), HistTitle.Data(), Nbins_SeedCharge,
                                      R_SeedCharge[0], R_SeedCharge[1]);
        ...
        HistTitle =
            TString("Analog #DeltaU for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_DeltaAnalogPositionU[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_DeltaPosition, R_DeltaPositionU[0],
                     R_DeltaPositionU[1]);
        ...
        HistTitle =
            TString("Analog #DeltaV for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_DeltaAnalogPositionV[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_DeltaPosition, R_DeltaPositionV[0],
                     R_DeltaPositionV[1]);
        ...
        HistTitle = TString("Analog #DeltaU vs #DeltaV for Thr = ") + TString(ytitle) +
                    TString(" #times Noise");
        h_DeltaAnalogPositionUV[ithr] = new TH2F(
            HistName.Data(), HistTitle.Data(), Nbins_DeltaPosition, R_DeltaPositionU[0],
            R_DeltaPositionU[1], Nbins_DeltaPosition, R_DeltaPositionV[0], R_DeltaPositionV[1]);
        ...
        HistTitle =
            TString("Digital #DeltaU for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_DeltaDigitalPositionU[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_DeltaPosition, R_DeltaPositionU[0],
                     R_DeltaPositionU[1]);
        ...
        HistTitle =
            TString("Digital #DeltaV for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_DeltaDigitalPositionV[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_DeltaPosition, R_DeltaPositionV[0],
                     R_DeltaPositionV[1]);
        ...
        HistTitle = TString("Digital #DeltaU vs #DeltaV for Thr = ") + TString(ytitle) +
                    TString(" #times Noise");
        h_DeltaDigitalPositionUV[ithr] = new TH2F(
            HistName.Data(), HistTitle.Data(), Nbins_DeltaPosition, R_DeltaPositionU[0],
            R_DeltaPositionU[1], Nbins_DeltaPosition, R_DeltaPositionV[0], R_DeltaPositionV[1]);
        ...
        HistTitle =
            TString("mean #DeltaU vs mult for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_MeanDigitalDeltaU_vs_Mult[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_MyMult, R_MyMult[0], R_MyMult[1]);
        ...
        HistTitle =
            TString("mean #DeltaV vs mult for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_MeanDigitalDeltaV_vs_Mult[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_MyMult, R_MyMult[0], R_MyMult[1]);
        ...
        HistTitle =
            TString("rms #DeltaU vs mult for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_RMSDigitalDeltaU_vs_Mult[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_MyMult, R_MyMult[0], R_MyMult[1]);
        ...
        HistTitle =
            TString("rms #DeltaV vs mult for Thr = ") + TString(ytitle) + TString(" #times Noise");
        h_RMSDigitalDeltaV_vs_Mult[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_MyMult, R_MyMult[0], R_MyMult[1]);
        ...
        HistTitle = TString("Trk distance to closest diode for all detected hits for Thr = ") +
                    TString(ytitle) + TString(" #times Noise");
        h_TrkDistToDiode_Detected[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_TrkDistToDiode, R_TrkDistToDiode[0],
                     R_TrkDistToDiode[1]);
        ...
        HistTitle = TString("Trk distance to closest diode for non-detected hits for Thr = ") +
                    TString(ytitle) + TString(" #times Noise");
        h_TrkDistToDiode_NonDetected[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_TrkDistToDiode, R_TrkDistToDiode[0],
                     R_TrkDistToDiode[1]);
        ...
        HistTitle =
            TString(
                "Efficiency vs Trk distance to closest diode for all detected hits for Thr = ") +
            TString(ytitle) + TString(" #times Noise");
        h_Effic_vs_TrkDistToDiode[ithr] =
            new TH1F(HistName.Data(), HistTitle.Data(), Nbins_TrkDistToDiode, R_TrkDistToDiode[0],
                     R_TrkDistToDiode[1]);
        ...
        HistTitle = TString("Trk U vs V position for all detected hits for Thr = ") +
                    TString(ytitle) + TString(" #times Noise");
        h_TrkDistToDiode2D_Detected[ithr] =
            new TH2F(HistName.Data(), HistTitle.Data(), Nbins_TrkDistToDiode2D,
                     R_TrkDistToDiode2DU[0], R_TrkDistToDiode2DU[1], Nbins_TrkDistToDiode2D,
                     R_TrkDistToDiode2DV[0], R_TrkDistToDiode2DV[1]);
        ...
        HistTitle = TString("Trk U vs V position for all detected hits for Thr = ") +
                    TString(ytitle) + TString(" #times Noise");
        h_TrkDistToDiode2D_NonDetected[ithr] =
            new TH2F(HistName.Data(), HistTitle.Data(), Nbins_TrkDistToDiode2D,
                     R_TrkDistToDiode2DU[0], R_TrkDistToDiode2DU[1], Nbins_TrkDistToDiode2D,
                     R_TrkDistToDiode2DV[0], R_TrkDistToDiode2DV[1]);
        ...
        for (int imult = 0; imult < NMults; imult++) {
            HistName = TString("h_DeltaDigitalPositionU_Thr") + long(ithr + 1) + TString("_mult") +
                       long(imult + 1);
            if (imult + 1 < NMults)
                HistTitle = TString("Digital #DeltaU mult =  ") + long(imult + 1) +
                            (" and for Thr = ") + TString(ytitle) + TString(" #times Noise");
            else
                HistTitle = TString("Digital #DeltaU mult >= ") + long(imult + 1) +
                            (" and for Thr = ") + TString(ytitle) + TString(" #times Noise");
            h_DeltaDigitalPositionU_vs_Mult[ithr][imult] =
                new TH1F(HistName.Data(), HistTitle.Data(), Nbins_DeltaPosition,
                         R_DeltaPositionU[0], R_DeltaPositionU[1]);
            ...
            HistName = TString("h_DeltaDigitalPositionV_Thr") + long(ithr + 1) + TString("_mult") +
                       long(imult + 1);
            if (imult + 1 < NMults)
                HistTitle = TString("Digital #DeltaV mult =  ") + long(imult + 1) +
                            (" and for Thr = ") + TString(ytitle) + TString(" #times Noise");
            else
                HistTitle = TString("Digital #DeltaV mult >= ") + long(imult + 1) +
                            (" and for Thr = ") + TString(ytitle) + TString(" #times Noise");
            h_DeltaDigitalPositionV_vs_Mult[ithr][imult] =
                new TH1F(HistName.Data(), HistTitle.Data(), Nbins_DeltaPosition,
                         R_DeltaPositionU[0], R_DeltaPositionU[1]);
            ...
            HistName = TString("h_TrkDistToDiode_Detected_Thr") + long(ithr + 1) +
                       TString("_mult") + long(imult + 1);
            if (imult + 1 < NMults)
                HistTitle =
                    TString("Trk distance to closest diode for detected hits for mult =  ") +
                    long(imult + 1) + (" and for Thr = ") + TString(ytitle) +
                    TString(" #times Noise");
            else
                HistTitle =
                    TString("Trk distance to closest diode for detected hits for mult >= ") +
                    long(imult + 1) + (" and for Thr = ") + TString(ytitle) +
                    TString(" #times Noise");
            h_TrkDistToDiode_Detected_vs_Mult[ithr][imult] =
                new TH1F(HistName.Data(), HistTitle.Data(), Nbins_TrkDistToDiode,
                         R_TrkDistToDiode[0], R_TrkDistToDiode[1]);
            h_TrkDistToDiode_Detected_vs_Mult[ithr][imult]->SetXTitle(
                "Trk dist to closest diode (#mum)");
            ...
            HistName = TString("h_TrkDistToDiode2D_Detected_Thr") + long(ithr + 1) +
                       TString("_mult") + long(imult + 1);
            if (imult + 1 < NMults)
                HistTitle = TString("Trk U vs V position for detected hits for mult =  ") +
                            long(imult + 1) + (" and for Thr = ") + TString(ytitle) +
                            TString(" #times Noise");
            else
                HistTitle = TString("Trk U vs V position for detected hits for mult >= ") +
                            long(imult + 1) + (" and for Thr = ") + TString(ytitle) +
                            TString(" #times Noise");
            h_TrkDistToDiode2D_Detected_vs_Mult[ithr][imult] =
                new TH2F(HistName.Data(), HistTitle.Data(), Nbins_TrkDistToDiode2D,
                         R_TrkDistToDiode2DU[0], R_TrkDistToDiode2DU[1], Nbins_TrkDistToDiode2D,
                         R_TrkDistToDiode2DV[0], R_TrkDistToDiode2DV[1]);
        ...
        cout << endl;
        cout << "Doing simulation for Threshold = " << Threshold_Times_Noise << " x Noise ("
             << ithr + 1 << " out of " << N_Thr_steps << ")" << endl;
        for (Int_t Nev = 0; Nev < Nevents; Nev++) {
            if (!((Nev + 1) % fPrintFreq)) {
            ...
            if (verbose) {
                ...
                cout << " - (theta,phi)  = (" << RandTheta * 180.0 / TMath::Pi() << ","
                     << RandPhi * 180.0 / TMath::Pi() << ") deg" << endl;
            ...
            for (Int_t ipix = 0; ipix < int(fPixelMap.size()); ipix++) {
                ...
                if (verbose) {
                    cout << "pixel " << ipix << ", "
                         ...
                         << "Digital Charge = " << fDigitalChargeMap[ipix] << endl;
                }
            ...
            if (NhitsToDraw[ithr] < Nhits_check && NhitsToDraw[ithr] < Nevents) {
                ...
                HistName = TString("h_ClusterShape_Analog_hit") + long(NhitsToDraw[ithr] + 1) +
                           TString("_Thr") + long(ithr + 1);
                HistTitle = TString("Analog 2D cluster for hit ") + long(NhitsToDraw[ithr] + 1) +
                            (" for Thr = ") + TString(ytitle) + TString(" #times Noise (") +
                            TString(ytitle2) + TString("#times10^{3} elec)");
                h_ClusterShape_Analog[NhitsToDraw[ithr]][ithr] =
                    new TH2F(HistName.Data(), HistTitle.Data(), Nbins_ClusterCheck_Col,
                             R_ClusterCheck_Col[0], R_ClusterCheck_Col[1], Nbins_ClusterCheck_Row,
                             R_ClusterCheck_Row[0], R_ClusterCheck_Row[1]);
                ...
                h_ClusterShape_Analog[NhitsToDraw[ithr]][ithr]->SetZTitle(
                    "Analog Charge (#times10^{3} electrons)");
                ...
                HistName = TString("h_ClusterShape_Digital_hit") + long(NhitsToDraw[ithr] + 1) +
                           TString("_Thr") + long(ithr + 1);
                HistTitle = TString("Digital 2D cluster for hit ") + long(NhitsToDraw[ithr] + 1) +
                            (" for Thr = ") + TString(ytitle) + TString(" #times Noise (") +
                            TString(ytitle2) + TString("#times10^{3} elec)");
                h_ClusterShape_Digital[NhitsToDraw[ithr]][ithr] =
                    new TH2F(HistName.Data(), HistTitle.Data(), Nbins_ClusterCheck_Col,
                             R_ClusterCheck_Col[0], R_ClusterCheck_Col[1], Nbins_ClusterCheck_Row,
                             R_ClusterCheck_Row[0], R_ClusterCheck_Row[1]);
                ...
                double rell_ppp =
                    0.015 * TMath::Max(Nbins_ClusterCheck_Col, Nbins_ClusterCheck_Row);
                double rell_pppUV = 0.015 * TMath::Max(Nbins_ClusterCheck_Col * GetPitchU(),
                                                       Nbins_ClusterCheck_Row * GetPitchV());
                ...
                InPostion[NhitsToDraw[ithr]][ithr] =
                    new TEllipse(InCol_ppp, InRow_ppp, rell_ppp, rell_ppp);
                ...
                OutPostion[NhitsToDraw[ithr]][ithr] =
                    new TEllipse(OutCol_ppp, OutRow_ppp, rell_ppp, rell_ppp);
                ...
                InPostionUV[NhitsToDraw[ithr]][ithr] =
                    new TEllipse(InputUpos, InputVpos, rell_pppUV, rell_pppUV);
                ...
                OutPostionUV[NhitsToDraw[ithr]][ithr] =
                    new TEllipse(OutputUpos, OutputVpos, rell_pppUV, rell_pppUV);
                ...
                for (Int_t ipix = 0; ipix < int(fPixelMap.size()); ipix++) {
                    ...
                    if (ipix + 1 > MaxPixelInCluster) {
                        cout << "WARNING: for Threshold = " << ytitle << " (thr_idx = " << ithr + 1
                             << "), evt = " << Nev + 1 << ", the index of pixel in cluster "
                             << ipix + 1 << " goes beyong allowed limit " << MaxPixelInCluster
                             << ". Cutting this cluster with size " << fPixelMap.size() << endl;
                        continue;
                    }

                    PixelPosition[NhitsToDraw[ithr]][ithr][NPixelsCheck[NhitsToDraw[ithr]][ithr]] =
                        TVector2(u, v);
                    PixelAnalogCharge[NhitsToDraw[ithr]][ithr]
                                     [NPixelsCheck[NhitsToDraw[ithr]][ithr]] =
                                         RoundOff(Decimals * fAnalogChargeMap[ipix] * 1.0e-3) /
                                         Decimals;
                    PixelDigitalCharge[NhitsToDraw[ithr]][ithr]
                                      [NPixelsCheck[NhitsToDraw[ithr]][ithr]] =
                                          fDigitalChargeMap[ipix];
                    NPixelsCheck[NhitsToDraw[ithr]][ithr]++;

                    int idx_col =
                        h_ClusterShape_Analog[NhitsToDraw[ithr]][ithr]->GetXaxis()->FindBin(col);
                    int idx_row =
                        h_ClusterShape_Analog[NhitsToDraw[ithr]][ithr]->GetYaxis()->FindBin(row);
                    h_ClusterShape_Analog[NhitsToDraw[ithr]][ithr]->SetBinContent(
                        idx_col, idx_row,
                        RoundOff(Decimals * fAnalogChargeMap[ipix] * 1.0e-3) / Decimals);

                    if (fDigitalChargeMap[ipix] < 1) continue;
                    h_ClusterShape_Digital[NhitsToDraw[ithr]][ithr]->SetBinContent(idx_col, idx_row,
                                                                                   1.0);
                }
            ...
            if (TMath::Abs(SumOfWeights_analog) > 1.0e-8 && IsDetected_analog) {
                ...
                if (verbose) {
                    cout << " Delta Analog Position (u,v) = (" << AveUpos - Position_analog_U << ","
                         << AveVpos - Position_analog_V << ") um" << endl;
                }

                h_DeltaAnalogPositionU[ithr]->Fill(AveUpos - Position_analog_U);
                h_DeltaAnalogPositionV[ithr]->Fill(AveVpos - Position_analog_V);
                h_DeltaAnalogPositionUV[ithr]->Fill(AveUpos - Position_analog_U,
                                                    AveVpos - Position_analog_V);
            }
            if (totalMult > 0) {
                ...
                if (verbose) {
                    cout << " Digital Position (u,v) = (" << AveUpos - Position_digital_U << ","
                         << AveVpos - Position_digital_V << ") um" << endl;
                }

                h_DeltaDigitalPositionU[ithr]->Fill(AveUpos - Position_digital_U);
                h_DeltaDigitalPositionV[ithr]->Fill(AveVpos - Position_digital_V);
                h_DeltaDigitalPositionUV[ithr]->Fill(AveUpos - Position_digital_U,
                                                     AveVpos - Position_digital_V);
                ...
                h_TrkDistToDiode2D_Detected_vs_Mult[ithr][mult_idx]->Fill(ShiftedPosition.X(),
                                                                          ShiftedPosition.Y());
            } else {
                h_TrkDistToDiode_NonDetected[ithr]->Fill(trk_dist_to_diode);
                h_TrkDistToDiode2D_NonDetected[ithr]->Fill(ShiftedPosition.X(),
                                                           ShiftedPosition.Y());
            }
    ...
    for (int ithr = 0; ithr < N_Thr_steps; ithr++) {
        ...
        f_DeltaAnalogPositionV[ithr] = new TF1(
            HistName.Data(), FitModel.Data(), -Fit_factor * h_DeltaAnalogPositionV[ithr]->GetRMS(),
        ...
        f_DeltaAnalogPositionU[ithr] = new TF1(
            HistName.Data(), FitModel.Data(), -Fit_factor * h_DeltaAnalogPositionU[ithr]->GetRMS(),
        ...
        f_DeltaDigitalPositionV[ithr] = new TF1(
            HistName.Data(), FitModel.Data(), -Fit_factor * h_DeltaDigitalPositionV[ithr]->GetRMS(),
        ...
        f_DeltaDigitalPositionU[ithr] = new TF1(
            HistName.Data(), FitModel.Data(), -Fit_factor * h_DeltaDigitalPositionU[ithr]->GetRMS(),
        ...
        for (int imult = 0; imult < NMults; imult++) {
            ...
            FuncName =
                TString("Fit_") + TString(h_DeltaDigitalPositionU_vs_Mult[ithr][imult]->GetName());
            f_DeltaDigitalPositionU_vs_Mult[ithr][imult] =
                new TF1(HistName.Data(), FitModel.Data(),
            ...
            f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->SetParameter(
                0, h_DeltaDigitalPositionU_vs_Mult[ithr][imult]->GetMean());
            f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->SetParName(0, "#mu (#mum)");
            f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->SetParameter(
                1, h_DeltaDigitalPositionU_vs_Mult[ithr][imult]->GetRMS());
            f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->SetParName(1, "#sigma (#mum)");
            f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->SetParameter(
                2, h_DeltaDigitalPositionU_vs_Mult[ithr][imult]->GetEntries());
            f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->SetParName(2, "N_{evts}");

            h_DeltaDigitalPositionU_vs_Mult[ithr][imult]->Sumw2();
            h_DeltaDigitalPositionU_vs_Mult[ithr][imult]->Fit(
                f_DeltaDigitalPositionU_vs_Mult[ithr][imult], "R");
            h_DeltaDigitalPositionU_vs_Mult[ithr][imult]->Draw();

            h_MeanDigitalDeltaU_vs_Mult[ithr]->SetBinContent(
                imult + 1, f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->GetParameter(0));
            h_MeanDigitalDeltaU_vs_Mult[ithr]->SetBinError(
                imult + 1, f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->GetParError(0));
            h_RMSDigitalDeltaU_vs_Mult[ithr]->SetBinContent(
                imult + 1, f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->GetParameter(1));
            h_RMSDigitalDeltaU_vs_Mult[ithr]->SetBinError(
                imult + 1, f_DeltaDigitalPositionU_vs_Mult[ithr][imult]->GetParError(1));
        }
        c1->Print(EPSName.Data());
        ...
        for (int imult = 0; imult < NMults; imult++) {
            ...
            FuncName =
                TString("Fit_") + TString(h_DeltaDigitalPositionV_vs_Mult[ithr][imult]->GetName());
            f_DeltaDigitalPositionV_vs_Mult[ithr][imult] =
                new TF1(HistName.Data(), FitModel.Data(),
            ...
            f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->SetParameter(
                0, h_DeltaDigitalPositionV_vs_Mult[ithr][imult]->GetMean());
            f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->SetParName(0, "#mu (#mum)");
            f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->SetParameter(
                1, h_DeltaDigitalPositionV_vs_Mult[ithr][imult]->GetRMS());
            f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->SetParName(1, "#sigma (#mum)");
            f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->SetParameter(
                2, h_DeltaDigitalPositionV_vs_Mult[ithr][imult]->GetEntries());
            f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->SetParName(2, "N_{evts}");

            h_DeltaDigitalPositionV_vs_Mult[ithr][imult]->Sumw2();
            h_DeltaDigitalPositionV_vs_Mult[ithr][imult]->Fit(
                f_DeltaDigitalPositionV_vs_Mult[ithr][imult], "R");
            h_DeltaDigitalPositionV_vs_Mult[ithr][imult]->Draw();

            h_MeanDigitalDeltaV_vs_Mult[ithr]->SetBinContent(
                imult + 1, f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->GetParameter(0));
            h_MeanDigitalDeltaV_vs_Mult[ithr]->SetBinError(
                imult + 1, f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->GetParError(0));
            h_RMSDigitalDeltaV_vs_Mult[ithr]->SetBinContent(
                imult + 1, f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->GetParameter(1));
            h_RMSDigitalDeltaV_vs_Mult[ithr]->SetBinError(
                imult + 1, f_DeltaDigitalPositionV_vs_Mult[ithr][imult]->GetParError(1));
        }
        c1->Print(EPSName.Data());
    ...
    for (int ithr = 0; ithr < N_Thr_steps; ithr++) {
        for (int ihit = 0; ihit < NhitsToDraw[ithr]; ihit++) {
            ...
            for (int i = 0; i < h_ClusterShape_Analog[ihit][ithr]->GetXaxis()->GetNbins() - 1;
                 i++) {
                double x = h_ClusterShape_Analog[ihit][ithr]->GetXaxis()->GetBinCenter(i + 1) +
                           0.5 * h_ClusterShape_Analog[ihit][ithr]->GetXaxis()->GetBinWidth(i + 1);
                vvv[i] = TLine(x, h_ClusterShape_Analog[ihit][ithr]->GetYaxis()->GetXmin(), x,
                               h_ClusterShape_Analog[ihit][ithr]->GetYaxis()->GetXmax());
                vvv[i].SetLineColor(1);
                vvv[i].SetLineWidth(1);
                vvv[i].SetLineStyle(2);
            }
            for (int i = 0; i < h_ClusterShape_Analog[ihit][ithr]->GetYaxis()->GetNbins() - 1;
                 i++) {
                double x = h_ClusterShape_Analog[ihit][ithr]->GetYaxis()->GetBinCenter(i + 1) +
                           0.5 * h_ClusterShape_Analog[ihit][ithr]->GetYaxis()->GetBinWidth(i + 1);
            ...
            for (int i = 0; i < NPixelsCheck[ihit][ithr]; i++) {
                ...
                HistName = TString("h_ClusterShape_AnalogUV_Hit") + long(ihit + 1) +
                           TString("_Thr") + long(ithr + 1) + TString("_pix") + long(i + 1);
                h_ClusterShape_AnalogUV[i] =
                    new TH2F(HistName.Data(), "", 1, u - 0.5 * GetPitchU(), u + 0.5 * GetPitchU(),
                             1, v - 0.5 * GetPitchV(), v + 0.5 * GetPitchV());
                h_ClusterShape_AnalogUV[i]->SetBinContent(1, 1, PixelAnalogCharge[ihit][ithr][i]);

                HistName = TString("h_ClusterShape_DigitalUV_Hit") + long(ihit + 1) +
                           TString("_Thr") + long(ithr + 1) + TString("_pix") + long(i + 1);
                h_ClusterShape_DigitalUV[i] =
                    new TH2F(HistName.Data(), "", 1, u - 0.5 * GetPitchU(), u + 0.5 * GetPitchU(),
                             1, v - 0.5 * GetPitchV(), v + 0.5 * GetPitchV());
                if (PixelDigitalCharge[ihit][ithr][i] > 0)
                    h_ClusterShape_DigitalUV[i]->SetBinContent(1, 1, 1);
                else
                    h_ClusterShape_DigitalUV[i]->SetBinContent(1, 1, -10);
            }
            ...
            TH2F h_ref(HistName.Data(), "", 100, RangeClusterU[0], RangeClusterU[1], 100,
                       RangeClusterV[0], RangeClusterV[1]);
            ...
            // PixelPosition[NhitsToDraw[ithr]][ithr][NPixelsCheck[NhitsToDraw[ithr]][ithr]]      =
            // TVector2(u,v);
            // PixelAnalogCharge[NhitsToDraw[ithr]][ithr][NPixelsCheck[NhitsToDraw[ithr]][ithr]]  =
            // RoundOff(Decimals*fAnalogChargeMap[ipix]*1.0e-3)/Decimals;
            // PixelDigitalCharge[NhitsToDraw[ithr]][ithr][NPixelsCheck[NhitsToDraw[ithr]][ithr]] =
            // fDigitalChargeMap[ipix]; NPixelsCheck[NhitsToDraw[ithr]][ithr]++;
            // InPostionUV[NhitsToDraw[ithr]][ithr]->SetFillColor(1);
